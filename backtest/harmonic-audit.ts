...
function runFullBacktest(candles: Candle[]): { trades: SimulatedTrade[]; harmonicTrades: SimulatedTrade[] } {
  ...
  // Refactor variant A, Фаза 5: computeSplitMetrics() больше не берёт
  // payout параметром — доходность считается от реального rMultiple
  // каждой сделки (см. metrics.ts).
  ...
  const forwardTest = computeForwardTestReport(trades, currentFreezeMs(LOGIC_CHANGE_LOG));
  ...
  if (harmonicTrades.length === 0) {
    ...
    console.log(`  Average Return: ${m.averageR >= 0 ? '+' : ''}${m.averageR.toFixed(2)}R`);
