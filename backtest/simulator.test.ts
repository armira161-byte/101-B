...
describe('simulate', () => {
  ...
  // Refactor variant A, Фаза 5 — rMultiple теперь реальная доходность
  // геометрии сделки, а не бинарный payout/-1. Алгебраическое следствие
  // позиционной модели (та же формула, что useDemoAccountStore.ts):
  // попадание именно в stopLoss всегда даёт rMultiple === -1 ровно, а не
  // "около -1" — это прямая проверка того, что simulator.ts действительно
  // считает R от геометрии входа/выхода, а не подставляет старую константу.
  it('every loss (stop_loss exit) has rMultiple exactly -1', () => {
    const candles = uptrendCandles(200);
    const trades = simulate(candles, {
      ...BASE_OPTIONS, symbol: 'BTCUSDT', timeframe: '1m',
    });
