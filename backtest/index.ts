...
async function main(): Promise<void> {
  ...
  // Refactor variant A, Фаза 5: единого --profit-percent больше нет —
  // computeSplitMetrics()/computeForwardTestReport() считают доходность и
  // точку безубыточности от реального rMultiple/averageWinR каждой сделки
  // (см. metrics.ts, change-registry.ts), а не от внешнего CLI-параметра.
  const split = computeSplitMetrics(trades);
  const forwardTest = computeForwardTestReport(trades, freezeAtMs);
