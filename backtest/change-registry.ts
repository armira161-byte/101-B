import { wilsonLowerBound } from '@/lib/pattern-reliability-calibration';
...
export const LOGIC_CHANGE_LOG: LogicChangeRecord[] = [
  ...
  {
    id: 'breakeven-position-model-aware',
    date: '2026-09-17',
    description:
      'Refactor variant A, Фаза 5. computeForwardTestReport больше не берёт breakevenWinRate из фиксированного profitPercent демо-счёта (экономика, удалённая Фазой 4) — точка безубыточности выводится из metrics.averageWinR той же выборки (p = 1/(averageWinR+1), т.к. loss всегда даёт rMultiple=-1 в позиционной модели). SimulatedTrade.rMultiple и вся computeMetrics()-семья переведены с бинарного WIN_R=profitPercent/100 на реальную геометрию сделки.',
    filesChanged: [
      'backtest/simulator.ts',
      'backtest/metrics.ts',
      'backtest/change-registry.ts',
      'src/decision/apply-spread.ts',
    ],
    frozenAtMs: Date.UTC(2026, 8, 17),
  },
];
...
//
// История: раньше здесь стоял breakevenWinRateFromProfitPercent(
// profitPercent) — точка безубыточности p = 100/(100+profitPercent),
// выведенная из ФИКСИРОВАННОЙ бинарно-опционной выплаты демо-счёта. Фаза 4
// удалила эту экономику из useDemoAccountStore.ts полностью (позиционный
// P&L: pnl = positionSize × (exitPrice − entryPrice) × sign) — payout
// демо-счёта («profitPercent» в его публичном API) больше НЕ участвует ни
// в одном реальном расчёте денег, это устаревший вход, см. комментарий в
// useDemoAccountStore.ts. Использовать его здесь для вердикта форвард-теста
// значило бы проверять текущую (позиционную) торговую логику по точке
// безубыточности МОДЕЛИ, которой в продукте больше нет.
//
// Новая точка безубыточности выводится из того, что реально происходит на
// этих сделках: при позиционной модели loss всегда даёт rMultiple === -1
// (это алгебраическое следствие формулы позиционного P&L — см.
// simulator.ts), поэтому breakeven — это классическая формула ожидаемого
// значения ставки с асимметричной выплатой:
//   p·avgWinR − (1−p)·1 = 0  =>  p = 1 / (avgWinR + 1)
// где avgWinR — средний R СРЕДИ РЕАЛЬНО ВЫИГРАННЫХ (metrics.averageWinR,
// Фаза 5, metrics.ts) сделок этой же выборки, а не внешний константный
// параметр. Если победных сделок ещё не было (avgWinR<=0 — averageWinR
// возвращает 0, когда wins.length===0, см. metrics.ts), точку
// безубыточности посчитать не от чего — возвращается 1 (100%), что
// корректно недостижимо и не завышает вердикт по построению.
export function breakevenWinRateFromAverageWinR(averageWinR: number): number {
  if (!Number.isFinite(averageWinR) || averageWinR <= 0) return 1;
  return 1 / (averageWinR + 1);
}

/**
 * Честная оценка "подтверждена ли текущая логика форвард-тестом", а не
 * ретроспективным бэктестом. Использует wilsonLowerBound из
 ...
 * Точка безубыточности (Фаза 5) больше не берёт profitPercent параметром —
 * см. breakevenWinRateFromAverageWinR() выше.
 */
export function computeForwardTestReport(
  ...
  const metrics = computeMetrics(forward);
  const decidedCount = metrics.wins + metrics.losses;
  const breakevenWinRate = breakevenWinRateFromAverageWinR(metrics.averageWinR);
