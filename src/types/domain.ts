...
export interface PatternResult {
  ...
  sweepLow?: number;
  sweepHigh?: number;
  // Shared by the growing "sweep/edge extreme + structural target" pattern
  // family (originally liquidity-sweep-reaction only, now also
  // liquidity-sweep base, order-block-breaker, strong-order-block-reaction,
  // macd-deceleration-continuation — see each detector's own comment for
  // what sweepLow/sweepHigh mean there) — nearest opposite-side OB/FVG edge
  // beyond entry, precomputed inside the detector (which has smartMoney in
  // scope) so the decision layer doesn't need its own copy of smartMoney to
  // place a structural take-profit. Null when no such zone was found.
  ...
  // liquidity-sweep (base) only — the local Trading Range boundaries (the
  // same recentHigh/recentLow lookback window the detector already computes
  // to find the swept extreme). Needed ONLY for setupType ===
  // 'reversal-at-key-level' (Wyckoff Spring/Upthrust): per
  // sltp-geometry-source-variant-A.md §3, that setup's target is the
  // OPPOSITE boundary of the same range the spring/upthrust occurred in —
  // not a smartMoney zone (that's the continuation setup's target instead).
  // See computeLiquiditySweepBaseTradeLevels in trade-levels.ts.
  rangeHigh?: number;
  rangeLow?: number;
  // fvg-return / fvg-rejection / fvg-breaker-block / fvg-nested only — the
  // zone's own top/bottom (for fvg-nested specifically: the M1-nested FVG's
  // own bounds, NOT the HTF zone's — the HTF zone's near boundary is the
  // target for that pattern, passed via oppositeZonePrice below despite the
  // name, since it plays the same "precomputed structural target" role).
  // Structural stop is this boundary ± buffer×ATR — see
  // computeFvgTradeLevels in trade-levels.ts and
  // sltp-geometry-source-variant-A.md §2.
  fvgTop?: number;
  fvgBottom?: number;
...
// BUGFIX (пре-существующая рассинхронизация, обнаружена и исправлена при
// аудите bolt.new-сборки — не связана с вариантом А): раньше здесь стояли
// atrMultiplier/stopLossPips/takeProfitPips/winRate — поля ДО того, как
// worker.ts::calibrateInWorker() был переписан ("РЕФАКТОРИНГ (бинарные
// опционы, Фаза 2/4)", см. compute/worker.ts) на модель "подбор expiryBars,
// максимизирующего accuracy направления". Тип тогда обновлён не был —
// worker.ts/useAnalyticsStore.ts/CalibrationPanel.tsx уже везде
// последовательно используют expiryBars/accuracy (см. подробный
// комментарий в useAnalyticsStore.ts у поля expiryBacktestAccuracy), только
// этот интерфейс продолжал описывать старую, нигде фактически не
// используемую форму (ни одно место в кодовой базе не читает
// CalibrationResult.atrMultiplier/.stopLossPips/.takeProfitPips/.winRate —
// проверено перед этой правкой). Приведено в соответствие с тем, что
// реально возвращает calibrateInWorker() и ожидают его потребители.
...
export const patternResultSchema = z.object({
  ...
  fvgTop: z.number().optional(),
  fvgBottom: z.number().optional(),
