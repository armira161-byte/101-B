  ...
  describe('setCalibrationResult', () => {
    it('sets calibration ready when totalTrades > 0', () => {
      useAnalyticsStore.getState().setCalibrationResult({
        symbolId: 'BTCUSDT', timeframe: '15m', expiryBars: 1,
        accuracy: 0.6, totalTrades: 10, calibratedAt: Date.now(),
      });
      ...
      expect(useAnalyticsStore.getState().expiryBacktestAccuracy).toBe(0.6);
    });

    // BUGFIX (независимый аудит, 2026-09-08): ATR-бэктест — историческая
    // симуляция, полностью отдельная от живого винрейта по реальным
    // демо-сделкам (см. expiryBacktestAccuracy у AnalyticsState и его
    ...
    it('sets expiryBacktestAccuracy to null when winRate is 0', () => {
      useAnalyticsStore.getState().setCalibrationResult({
        symbolId: 'BTCUSDT', timeframe: '15m', expiryBars: 1,
        accuracy: 0, totalTrades: 5, calibratedAt: Date.now(),
      });
      expect(useAnalyticsStore.getState().expiryBacktestAccuracy).toBeNull();
    });
