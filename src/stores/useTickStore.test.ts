...
vi.mock('@/decision/engine', () => ({
  DecisionEngine: vi.fn().mockImplementation(() => ({
    ...
    getLastRejectionReason: vi.fn().mockReturnValue(null),
  })),
}));
...
vi.mock('@/decision/outcome-scheduler', () => ({
  ...
  getCandlesAfterSignal: vi.fn((allCandles: { time: number }[], signalTime: number) =>
    allCandles.filter((c) => c.time > signalTime)),
}));
...
function makeSignal(overrides: Partial<Signal> & { id: string; time: number }): Signal {
  return {
    ...
    score: 3, calibratedProbability: null, entryPrice: 100, stopLoss: 99, takeProfit: 101.5, reason: 'test', indicators: {} as unknown as Signal['indicators'],
    pattern: null, outcome: 'pending', frozenAt: null, isRevised: false,
    isPreClose: false, revisionNote: null, barsToResolve: 5, spread: null,
    // BUGFIX (Фаза 5, попутно обнаружено): было expiryBars/
    // estimatedPatternMaturityBars — этих полей в Signal больше нет.
