...
export async function handleCandle(
  ...
  if (last && last.time === candle.time) {
    ...
    if (last) {
      ...
      if (get().lastCandleCloseAtMs !== prevCloseMs) {
        ...
        const candlesWithNew = [...candles, candle];
        useDemoAccountStore.getState().checkExpiries(candlesWithNew, prevCloseMs, get().activeSymbolId, get().activeTimeframe);
  ...
  if (isClosed) {
    maybeCompute(set, get);
    useDemoAccountStore.getState().checkExpiries(candles, (candle.time + TIMEFRAME_SECONDS[state.activeTimeframe]) * 1000, state.activeSymbolId, state.activeTimeframe);
...
function handleTick(
  ...
  if (throttled) {
    ...
    // Тиковая проверка НЕ должна пропускать тики из-за троттлинга UI-
    // обновлений (TICK_THROTTLE_MS=200мс — этого достаточно, чтобы
    // реальное касание stopLoss/takeProfit было пропущено несколько раз
    // подряд при частом потоке тиков, если проверять только не-
    // троттленные тики). Троттлинг существует для дорогих операций
    // (пересчёт индикаторов, ре-рендер), а не для контроля риска.
    useDemoAccountStore.getState().checkTickLevel(tick, get().activeSymbolId, get().activeTimeframe);
    return;
  }
  ...
  // Refactor variant A, Фаза 3/4: тиковая проверка формирующейся свечи —
  // резолвит демо-сделку РАНЬШЕ закрытия свечи, если уровень уже задет по
  // факту котировки (см. checkTickLevel в useDemoAccountStore.ts и
  // resolveOutcomeByTick в decision/resolve-by-levels.ts). Дёшево при
  // отсутствии открытых сделок на активном инструменте — стор сам
  // фильтрует и рано выходит.
  useDemoAccountStore.getState().checkTickLevel(tick, get().activeSymbolId, get().activeTimeframe);
