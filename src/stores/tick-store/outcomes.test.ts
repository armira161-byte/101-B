...
// BUGFIX (Фаза 5, попутно обнаружено): эта фикстура была рассинхронизирована
// с текущим Signal (types/domain.ts) ещё до варианта А — не хватало
// обязательных stopLoss/takeProfit/recommendedExpiry (вместо них стояли
// несуществующие в типе expiryBars/estimatedPatternMaturityBars), и (что
// важнее для этого файла) резолв здесь происходит через реальный
// OutcomeScheduler.resolveOutcome(), который с Фазы 3 проверяет
// high/low каждой свечи против stopLoss/takeProfit, а не цену закрытия.
// Все сценарии в этом файле рассчитаны на один и тот же нарратив ("цена
// прошла от 100 к 108 после входа на 100 = победа для buy") — takeProfit
// подобран так, чтобы это по-прежнему было победой при резолве по уровню
// (105 < 108 = high единственной последующей свечи), а не только по close,
// как раньше.
...
// Плоская свеча (open=high=low=close): при резолве по уровню важно только
// пересекает ли high/low takeProfit/stopLoss — 105 < 108, поэтому такая
// свеча по-прежнему однозначно засчитывается как take_profit ('win').
