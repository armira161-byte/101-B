...
export function maybeResolveOutcomes(
  ...
  sched.onCandleClosed(state.candles, (resolved, signal) => {
    ...
    // исход непроторгованного сигнала.
    //
    // Refactor variant A, Фаза 5 (синхронизация с Фазой 3): раньше здесь
    // считался expiryClosePrice = candlesAfter[0]?.close — цена закрытия
    // РОВНО следующей свечи. Это был устаревший артефакт close-модели
    // резолва: resolved (из OutcomeScheduler, Фаза 3) теперь резолвится по
    // касанию уровня и может закрыться на любом баре в пределах
    // паттерн-специфичного горизонта, а не обязательно на первом же —
    // candlesAfter[0].close сравнивал спред с движением до СЛУЧАЙНОЙ
    // промежуточной свечи, а не с реальной ценой выхода сделки. Теперь
    // передаём сам resolved (exitPrice/exitReason уже посчитаны
    // resolveOutcomeByLevels) — та же функция и то же сравнение, что и в
    // useDemoAccountStore.ts/backtest/simulator.ts. applySpreadToOutcome()
    // больше не понижает outcome — см. её комментарий; adjusted.outcome
    // здесь равен resolved.outcome, оставлен явно, чтобы не потерять
    // spreadCostR-диагностику ниже, если в будущем recordOutcome() станет
    // от неё зависеть.
    ...
    // (тесты, старые записи) — resolveOutcome() в outcome-scheduler.ts
    // всегда заполняет оба поля для любого non-null результата, поэтому
    // этот guard не должен срабатывать на живых данных; он здесь только
    // чтобы не проваливать типизацию applySpreadToOutcome() опциональными
    // полями.
    if (resolved.exitPrice === undefined || resolved.exitReason === undefined) {
      return;
    }
