...
interface AnalyticsState {
  ...
  // expiryBars на истории). Отдельная фича от ML-калибровки, раньше делила
  // с ней одно и то же поле calibrationSampleCount.
  expiryBacktestTrades: number;
  // BUGFIX (независимый аудит, 2026-09-08): accuracy бэктеста экспирации
  ...
  // пересчитываемые из signals") — нажатие "Калибровать" временно
  // подменяло бы этот живой винрейт историческим, пока следующий
  // резолвнутый сигнал не вызывал recomputeStats() заново.
  // Тот же класс бага, что уже был исправлен для calibrationSampleCount
  // (см. mlSampleCount/expiryBacktestTrades/liveCompletedCount выше) — здесь
  // пропущен при том фиксе. Результат отображается напрямую из
  // calibrationResult.accuracy (см. CalibrationPanel.tsx), это поле — для
  // остальных мест, которым может понадобиться именно accuracy бэктеста
  // экспирации отдельно от живого.
  expiryBacktestAccuracy: number | null;
...
export const useAnalyticsStore = create<AnalyticsState>()(
  persist(
    (set, get) => ({
      ...
      mlSampleCount: 0,
      mlReady: false,
      expiryBacktestTrades: 0,
      expiryBacktestAccuracy: null,
      ...
      setCalibrationResult: (result) => {
        if (result) {
          set({
            ...
            // свой винрейт ТОЛЬКО в expiryBacktestAccuracy — тот же принцип
            // разделения полей, что уже применён к expiryBacktestTrades ниже.
            expiryBacktestAccuracy: result.accuracy > 0 ? result.accuracy : null,
            // Этап 1 аудита, п.3: это бэктест экспирации (подбор
            // expiryBars на истории) — отдельная фича от ML-калибровки,
            // пишет ТОЛЬКО в своё поле, не в mlSampleCount/mlReady.
            expiryBacktestTrades: result.totalTrades,
          });
        } else {
          set({
            calibrationResult: null,
