...
// РЕФАКТОРИНГ (бинарные опционы, Фаза 2): продуктовое решение из
// pattern-audit-checklist.md (строки 17 и 22) — Doji и Spinning Top по
// природе паттерны нерешительности, без направленного смещения по всем
// сверенным источникам; их текущая directional-логика в детекторах
// (доджи — "против предшествующего тренда", волчок — "по цвету последней
// свечи, close vs open") — эвристика, а не подтверждённый сигнал, и не
// годится как самостоятельный триггер для входа в бинарный опцион с
// фиксированной экспирацией. Остаются toggle'ами в UI/детекторами (не
// удалены — могут быть полезны для будущего исследования как фильтр
// контекста, не как самостоятельный сигнал), но выключены по умолчанию
// для НОВЫХ и существующих пользователей (см. migrate() version < 15
// ниже), пока не принято отдельное продуктовое решение об их включении.
const DIRECTIONALLY_UNRELIABLE_PATTERNS: readonly PatternName[] = ['doji', 'spinning-top'];
...
function defaultActiveFeatures(): { patterns: string[]; indicators: string[] } {
  return {
    patterns: ALL_PATTERNS.filter((p) => !DIRECTIONALLY_UNRELIABLE_PATTERNS.includes(p)),
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      activePatterns: ALL_PATTERNS.filter((p) => !DIRECTIONALLY_UNRELIABLE_PATTERNS.includes(p)),
    ...
    }),
    {
      ...
      migrate: (persisted, version) => {
        ...
        if (version < 14) {
          ...
          //    РЕФАКТОРИНГ (бинарные опционы, Фаза −1): harmonicMinRR
          //    backfill убран — поле удалено из IndicatorConfig целиком
          //    (minRR-гейт для структурного TP не имеет смысла без TP).
          //    Если старое значение осталось в localStorage — это
          //    безвредный лишний ключ, ничем больше не читается.
          const ind = (s.indicators ?? {}) as Record<string, number>;
        ...
        if (version < 15) {
          // РЕФАКТОРИНГ (бинарные опционы, Фаза 2): продуктовое решение —
          // Doji/Spinning Top выключаются по умолчанию как самостоятельный
          // направленный триггер (см. DIRECTIONALLY_UNRELIABLE_PATTERNS
          // выше и pattern-audit-checklist.md, строки 17/22). В отличие от
          // version < 9/12/14 (аддитивный домердж НОВЫХ паттернов), здесь
          // наоборот — явно убираем два конкретных паттерна из уже
          // сохранённого activePatterns существующих пользователей, если
          // они там есть (у новых пользователей их и так не будет —
          // см. defaultActiveFeatures()).
          const currentPatterns = Array.isArray(s.activePatterns) ? (s.activePatterns as string[]) : [];
          s.activePatterns = currentPatterns.filter((p) => !DIRECTIONALLY_UNRELIABLE_PATTERNS.includes(p as PatternName));
        }
        return s;
      },
    },
