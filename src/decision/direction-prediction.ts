import type { Candle, DirectionComponents, SignalDirection, Snapshot, SignalComponentToggles, FeatureName, SignalFactor, SignalFactorKind, Timeframe } from '@/types/domain';
...
export function computeDirectionScore(
  ...
  // до фикса.
  symbolId?: string,
  // РЕФАКТОРИНГ (бинарные опционы, "пересчёт весов по паттерну и
  // таймфрейму"): та же логика обратной совместимости, что у symbolId
  // выше — опционален, последний параметр, старые вызовы (тесты) без
  // изменений получают прежнее поведение (getReliabilityMultiplier без
  // timeframe пропускает самый специфичный слой).
  timeframe?: Timeframe,
  ...
  if (selection) {
    ...
    const reliabilityMultiplier = getReliabilityMultiplier(topPattern.name, symbolId, timeframe);
