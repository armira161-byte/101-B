...
describe('buildSignal', () => {
  ...
  // Refactor variant A, Фаза 2/3: горизонт удержания стал
  // паттерн-специфичным (pattern-horizon.ts), а не одной глобальной
  // константой. Значение из params остаётся только фолбэком для сигналов
  // без распознанного паттерна — именно этот случай проверяет тест выше
  // (makeSnapshot кладёт patterns: []).
  it('derives barsToResolve from the pattern, overriding the globally-passed constant', () => {
    const candles = makeCandles(true);
    const snap = makeSnapshot(candles, {
      rsi: 25, emaFast: 110, emaSlow: 100, macdHistogram: 1, atr: 2,
      bollingerUpper: 115, bollingerMiddle: 105, bollingerLower: 95,
    ...
    // 'bullish-engulfing' — короткий горизонт по чек-листу;
    // 'harmonic-pattern' — длинный. Оба должны разойтись с переданной 5.
    const shortPattern: PatternResult = {
      name: 'bullish-engulfing', direction: 'buy', confidence: 0.8, strength: 'strong', time: 0,
      sweepLow: lastClose - 3, sweepHigh: lastClose - 1,
    };
    const longPattern: PatternResult = {
      name: 'harmonic-pattern', direction: 'buy', confidence: 0.8, strength: 'strong', time: 0,
    };
    const cfg: IndicatorConfig = { ...CONFIG, scoreThreshold: 0 };

    const shortSignal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: cfg, atrMultiplier: 2,
      activeFeatures: [], snapshot: { ...snap, patterns: [shortPattern] },
      calibration: null, tick: null, barsToResolve: 5,
    });
    const longSignal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: cfg, atrMultiplier: 2,
      activeFeatures: [], snapshot: { ...snap, patterns: [longPattern] },
      calibration: null, tick: null, barsToResolve: 5,
    });

    expect(shortSignal).not.toBeNull();
    expect(longSignal).not.toBeNull();
    expect(shortSignal!.barsToResolve).toBe(maxHorizonBarsForPattern('bullish-engulfing'));
    expect(longSignal!.barsToResolve).toBe(maxHorizonBarsForPattern('harmonic-pattern'));
    // Главное: многобаровый паттерн получает строго большее окно, чем
    // импульсный — то, чего единая константа 5 не давала в принципе.
    expect(longSignal!.barsToResolve).toBeGreaterThan(shortSignal!.barsToResolve);
  });
...
// Refactor variant A, Фаза 0' группа 2, п.1: same structural SL/TP wiring
// as liquidity-sweep-reaction above, now also applied to the base
// (non-reaction) liquidity-sweep pattern — previously this pattern fell
// through to the generic estimateTradeLevels() despite the detector already
// finding a structurally meaningful swept extreme.
describe('buildSignal — liquidity-sweep structural SL/TP (Фаза 0\' группа 2, п.1, редакция 3)', () => {
  const LS_CONFIG: IndicatorConfig = { ...CONFIG, scoreThreshold: 0 };

  function lsPattern(overrides: Partial<PatternResult> = {}): PatternResult {
    return {
  ...
  it('places the stop beyond the sweep bar extreme, not at a fixed ATR multiple from entry', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 5;
    const snap = buySnapshot(candles);
    snap.patterns = [lsPattern({ sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice: null })];

    const signal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: LS_CONFIG, atrMultiplier: 2,
      activeFeatures: [], snapshot: snap, calibration: null, tick: null, barsToResolve: 5,
    });
    expect(signal).not.toBeNull();
    // atr=2, buffer default 0.1 → stopLoss = sweepLow - 0.2, independent of
    // atrMultiplier (which is what the generic estimateTradeLevels fallback
    // would have used instead). Same for both setupTypes — only the target
    // differs (see tests below).
    expect(signal!.stopLoss).toBeCloseTo(sweepLow - 0.2, 6);
  });

  it("setupType='continuation': targets the nearest opposite liquidity zone via smartMoney, not the Trading Range", () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 5;
    const oppositeZonePrice = lastClose + 50; // comfortably clears MIN_RR
    const snap = buySnapshot(candles);
    snap.patterns = [lsPattern({
      sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice,
      setupType: 'continuation',
      // Deliberately far away — must be ignored for a continuation setup.
      rangeHigh: lastClose + 1000, rangeLow: lastClose - 1000,
    })];
  ...
  it("setupType='reversal-at-key-level' (Wyckoff Spring): targets the opposite Trading Range boundary, ignoring smartMoney", () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 5;
    const rangeHigh = lastClose + 60; // comfortably clears MIN_RR from entry
    const snap = buySnapshot(candles);
    snap.patterns = [lsPattern({
      sweepLow, sweepHigh: lastClose - 1,
      setupType: 'reversal-at-key-level',
      rangeHigh, rangeLow: sweepLow - 10,
      // Deliberately present but must be ignored for this setupType.
      oppositeZonePrice: lastClose + 5,
    })];
    ...
    expect(signal).not.toBeNull();
    // Buy-side spring → target is the TOP of the range (rangeHigh), not
    // oppositeZonePrice.
    expect(signal!.takeProfit).toBe(rangeHigh);
  });

  it('falls back to the generic estimator when the pattern lacks setupType/range fields (e.g. a stale cached result)', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    // No setupType/rangeHigh/rangeLow supplied — simulates a pattern result
    // computed before this refactor added these fields.
    snap.patterns = [lsPattern({ setupType: undefined, rangeHigh: undefined, rangeLow: undefined })];
...
describe("buildSignal — order-block-breaker structural SL/TP (Фаза 0' группа 2, sltp-geometry-source-variant-A.md §1)", () => {
  const OBB_CONFIG: IndicatorConfig = { ...CONFIG, scoreThreshold: 0 };

  function obbPattern(overrides: Partial<PatternResult> = {}): PatternResult {
    return {
  ...
  it('places the stop beyond the liquidity-sweep candle, not a fixed ATR multiple from entry', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 6;
    const snap = buySnapshot(candles);
    snap.patterns = [obbPattern({ sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice: null })];

    const signal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: OBB_CONFIG, atrMultiplier: 2,
      activeFeatures: [], snapshot: snap, calibration: null, tick: null, barsToResolve: 5,
    });
    expect(signal).not.toBeNull();
    expect(signal!.stopLoss).toBeCloseTo(sweepLow - 0.2, 6);
  ...
  it('targets the nearest opposite smartMoney zone when it clears the minimum R:R', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 6;
    const oppositeZonePrice = lastClose + 60;
    const snap = buySnapshot(candles);
    snap.patterns = [obbPattern({ sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice })];
  ...
  it('falls back to the generic estimator when the sweep candle was not found by the detector', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [obbPattern({ sweepLow: undefined, sweepHigh: undefined })];
...
describe("buildSignal — macd-deceleration-continuation structural SL/TP (Фаза 0' группа 2)", () => {
  const MACD_CONFIG: IndicatorConfig = { ...CONFIG, scoreThreshold: 0 };

  function macdPattern(overrides: Partial<PatternResult> = {}): PatternResult {
    return {
  ...
  it('places the stop beyond the pause candle low, not a fixed ATR multiple from entry', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 4;
    const snap = buySnapshot(candles);
    snap.patterns = [macdPattern({ sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice: null })];

    const signal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: MACD_CONFIG, atrMultiplier: 2,
      activeFeatures: [], snapshot: snap, calibration: null, tick: null, barsToResolve: 5,
    });
    expect(signal).not.toBeNull();
    expect(signal!.stopLoss).toBeCloseTo(sweepLow - 0.2, 6);
  ...
  it('falls back to the generic estimator when the pause candle extremes are missing', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [macdPattern({ sweepLow: undefined, sweepHigh: undefined })];
...
describe("buildSignal — FVG family structural SL/TP (Фаза 0' группа 2, sltp-geometry-source-variant-A.md §2)", () => {
  ...
  it.each(['fvg-return', 'fvg-rejection', 'fvg-breaker-block', 'fvg-nested'])(
    '%s: places the stop beyond the FVG zone bottom, not a fixed ATR multiple from entry',
    (name) => {
      const candles = makeCandles(true);
      const lastClose = candles[candles.length - 1].close;
      ...
      expect(signal).not.toBeNull();
      expect(signal!.stopLoss).toBeCloseTo(fvgBottom - 0.2, 6);
    },
  );

  it('targets the precomputed structural candidate when it clears the minimum R:R', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const fvgBottom = lastClose - 4;
    const oppositeZonePrice = lastClose + 50;
    const snap = buySnapshot(candles);
    snap.patterns = [fvgPattern('fvg-return', { fvgBottom, fvgTop: lastClose - 1, oppositeZonePrice })];
  ...
  it('falls back to the generic estimator when the pattern lacks fvgTop/fvgBottom', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [fvgPattern('fvg-rejection', { fvgTop: undefined, fvgBottom: undefined })];
...
describe("buildSignal — order-block-nested structural SL/TP (Фаза 0' группа 2, OB counterpart of fvg-nested)", () => {
  const OBN_CONFIG: IndicatorConfig = { ...CONFIG, scoreThreshold: 0 };

  function obnPattern(overrides: Partial<PatternResult> = {}): PatternResult {
    return {
  ...
  it('places the stop beyond the inner M1 OB boundary, not a fixed ATR multiple from entry', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 4;
    const snap = buySnapshot(candles);
    snap.patterns = [obnPattern({ sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice: null })];

    const signal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: OBN_CONFIG, atrMultiplier: 2,
      activeFeatures: [], snapshot: snap, calibration: null, tick: null, barsToResolve: 5,
    });
    expect(signal).not.toBeNull();
    expect(signal!.stopLoss).toBeCloseTo(sweepLow - 0.2, 6);
  ...
  it('targets the containing HTF zone boundary when it clears the minimum R:R', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 4;
    const oppositeZonePrice = lastClose + 50;
    const snap = buySnapshot(candles);
    snap.patterns = [obnPattern({ sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice })];
  ...
  it('falls back to the generic estimator when the pattern lacks sweepLow/sweepHigh', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [obnPattern({ sweepLow: undefined, sweepHigh: undefined })];
...
describe("buildSignal — candlestick family structural SL/TP (Фаза 0' группа 3, price-action convention)", () => {
  ...
  it.each([
    'hammer', 'shooting-star', 'inverted-hammer', 'hanging-man', 'marubozu-bullish', 'marubozu-bearish',
    ...
    '%s: places the stop beyond the formation low, not a fixed ATR multiple from entry',
    (name) => {
      const candles = makeCandles(true);
      const lastClose = candles[candles.length - 1].close;
  ...
  it('targets the precomputed structural/measured-move candidate when it clears the minimum R:R', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 3;
    const oppositeZonePrice = lastClose + 40;
    const snap = buySnapshot(candles);
    snap.patterns = [candlePattern('hammer', { sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice })];
  ...
  it('falls back to the generic estimator when the pattern lacks sweepLow/sweepHigh', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [candlePattern('hammer', { sweepLow: undefined, sweepHigh: undefined })];
  ...
  it('does not apply candlestick-family geometry to Doji/Spinning Top (deliberately excluded — no directional thesis of their own)', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [candlePattern('doji', { sweepLow: 90, sweepHigh: 95 })];
    ...
    expect(signal).not.toBeNull();
    // Falls to the generic estimator, not computeCandlestickTradeLevels —
    // stopLoss must NOT sit at sweepLow-derived ATR-buffer distance.
    expect(signal!.stopLoss).not.toBeCloseTo(90 - 0.2, 6);
  });
});
...
describe("buildSignal — Three Methods structural SL/TP (Фаза 0' группа 4, measure rule)", () => {
  ...
  it('rising-three-methods: places the stop beyond the 4-candle formation low, not a fixed ATR multiple from entry', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 7;
    const snap = buySnapshot(candles);
    snap.patterns = [tmPattern('rising-three-methods', {
      sweepLow, sweepHigh: lastClose - 1, oppositeZonePrice: null,
    })];

    const signal = buildSignal({
      symbolId: 'BTCUSDT', timeframe: '15m', candles, config: TM_CONFIG, atrMultiplier: 2,
      activeFeatures: [], snapshot: snap, calibration: null, tick: null, barsToResolve: 5,
    });
    expect(signal).not.toBeNull();
    expect(signal!.stopLoss).toBeCloseTo(sweepLow - 0.2, 6);
  ...
  it('rising-three-methods: targets the precomputed measured-move projection when it clears the minimum R:R', () => {
    const candles = makeCandles(true);
    const lastClose = candles[candles.length - 1].close;
    const sweepLow = lastClose - 7;
    const oppositeZonePrice = lastClose + 60;
  ...
  it('falls back to the generic estimator when the pattern lacks sweepLow/sweepHigh', () => {
    const candles = makeCandles(true);
    const snap = buySnapshot(candles);
    snap.patterns = [tmPattern('rising-three-methods', { sweepLow: undefined, sweepHigh: undefined })];
