...
export class DecisionEngine {
  ...
  // РЕФАКТОРИНГ ("долго нет сигналов — проверить модуль"): раньше evaluate()
  // на любом из ~12 гейтов (session filter, warmup, no-direction, score <
  // threshold, ATR/spread invalid, priority-ramp, requireStrong+strength,
  // cooldown/chop-guard/serial-repeat) просто возвращал null — снаружи
  // невозможно было отличить "рынок сейчас реально без сетапов" от "гейт X
  // блокирует конкретный, иначе валидный сигнал уже T времени подряд".
  // lastRejectionReason — не влияет на сам сигнал/резолв, чисто
  // диагностическое поле для UI (см. useTickStore.ts/SignalFeed.tsx),
  // читаемое через getLastRejectionReason(). Не покрывает абсолютно каждый
  // return null в buildSignal() построчно (см. signal-builder.ts — там
  // сигнатура функции менять не стали, чтобы не расширять blast radius на
  // 30+ существующих тестов), но покрывает главные АУДИТ-НАХОДКУ кандидаты
  // на "надолго залипающую" блокировку: requireStrong (мартингейл/loss-
  // streak-guard включены по умолчанию — самый вероятный кандидат на
  // "часами нет сигналов", если рынок не даёт 'strong' сетапов),
  // cooldown/chop-guard/serial-repeat, отключённые источники сигнала и
  // общий факт "buildSignal ничего не вернул" (session filter/score ниже
  // порога/невалидный ATR или спред объединены в один общий текст, т.к.
  // сами по себе, по отдельности, не являются признаком поломки — это
  // нормальная, ожидаемая работа стратегии на большинстве баров).
  ...
  async evaluate(
    ...
    if (!hasEnabledSource) {
      ...
      this.lastRejectionReason = 'Все источники сигнала отключены в настройках (структура/зоны/ликвидность/триггер/индикатор/BOS/MACD)';
      return null;
    }
    ...
    if (!finalSignal) {
      // requireStrong raises the EFFECTIVE score threshold inside buildSignal
      // itself (see effectiveScoreThreshold above) — so when it's active, a
      // null here may already be the martingale/loss-streak requirement
      // biting (not just "no setup at all"), even though the separate
      // defense-in-depth check below never gets a non-null signal to reject.
      // From outside buildSignal there's no way to tell "no setup found at
      // any threshold" apart from "a setup existed below 'strong' but above
      // the user's own threshold" — but requireStrong being active is still
      // the most informative, actionable thing to tell the user in either
      // case (turning it off is the one thing they can try).
      this.lastRejectionReason = requireStrong
        ? 'Сигнал не сформирован: включена "Система мартингейла" и/или сработала защита от серии убытков — порог поднят до уровня "сильного" сигнала, и на этом баре ни один сетап его не достиг'
        : 'Ни один паттерн/сетап не прошёл score-порог, сессионный фильтр, ADX-гейт режима или спред-гейт на этом баре (обычная работа стратегии — не обязательно проблема, если это не длится необычно долго)';
    }
    ...
    if (finalSignal && requireStrong && finalSignal.strength !== 'strong') {
      ...
      this.lastRejectionReason = `Сигнал силы "${finalSignal.strength === 'moderate' ? 'средний' : 'слабый'}" найден, но подавлен: включена "Система мартингейла" и/или сработала защита от серии убытков — сейчас пропускаются только "сильные" сигналы`;
      finalSignal = null;
    }
    ...
    if (finalSignal) {
      ...
      if (suppressed || chopped || serialRepeat) {
        ...
        this.lastRejectionReason = chopped
          ? 'Сигнал найден, но подавлен chop-guard: в этой ценовой зоне за последние 15 минут уже было 2+ сигнала в разных направлениях ("пила")'
          : serialRepeat
            ? 'Сигнал найден, но подавлен серийным гейтом: уже несколько сигналов одного направления в этой зоне без подтверждённой смены структуры'
            : 'Сигнал найден, но подавлен кулдауном: в этой зоне уже есть недавний ещё не резолвленный сигнал того же направления';
        finalSignal = null;
      }
    }
