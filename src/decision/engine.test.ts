...
describe('OutcomeScheduler', () => {
  ...
  // stopLoss/takeProfit в пределах signal.barsToResolve, через общий
  // resolve-by-levels.ts (тот же модуль, что у бэктеста и — после Фазы 4 —
  // у демо-счёта). Предыдущая редакция этих тестов фиксировала
  // промежуточную close-модель; теперь они переписаны под уровни.
  //
  // Осторожно с фикстурой: makeSignal() по умолчанию даёт stopLoss: 90 /
  // takeProfit: 120, что корректно только для BUY. Для sell-кейсов уровни
  // задаются явно (стоп ВЫШЕ входа, цель НИЖЕ) — иначе уровни
  // перевёрнуты, и resolveOutcomeByLevels справедливо считает такую свечу
  // неоднозначной (задеты оба), а не чистой победой.
  it('resolveOutcome detects a buy win when high reaches takeProfit', () => {
  ...
  it('resolveOutcome detects a buy loss when low reaches stopLoss', () => {
    const signal = makeSignal({ direction: 'buy', entryPrice: 100 });
    const candles: Candle[] = [
  ...
  it('resolveOutcome detects a sell win when low reaches takeProfit', () => {
    const signal = makeSignal({ direction: 'sell', entryPrice: 100, stopLoss: 110, takeProfit: 80 });
    const candles: Candle[] = [
  ...
  it('resolveOutcome detects a sell loss when high reaches stopLoss', () => {
    const signal = makeSignal({ direction: 'sell', entryPrice: 100, stopLoss: 110, takeProfit: 80 });
    const candles: Candle[] = [
  ...
  it('resolveOutcome stays pending while neither level is touched and the horizon is not exhausted', () => {
    // Раньше (close-модель) равенство close === entryPrice трактовалось как
    // 'timeout' немедленно. Теперь close вообще не участвует в решении:
    // ни стоп (90), ни цель (120) не задеты, а из 5 баров горизонта прошёл
    // только один — сделка честно остаётся открытой.
    const signal = makeSignal({ direction: 'buy', entryPrice: 100 });
    const candles: Candle[] = [
      { time: 2000, open: 100, high: 101, low: 99, close: 100, volume: 10 },
    ];
    expect(resolveOutcome(signal, candles)).toBeNull();
  });

  it('resolveOutcome keeps scanning later candles within the horizon instead of locking in the first', () => {
    // Раньше резолв происходил строго на первой свече, и исход был 'win'
    // по её close (105 > 100). Теперь первая свеча не задевает уровней, а
    // вторая проваливается на 50 — ниже стопа 90, и это реальный исход.
    const signal = makeSignal({ direction: 'buy', entryPrice: 100 });
    const candles: Candle[] = [
  ...
  it('resolveOutcome returns timeout once the whole horizon passes untouched', () => {
    const signal = makeSignal({ direction: 'buy', entryPrice: 100, barsToResolve: 2 });
    const candles: Candle[] = [
      { time: 2000, open: 100, high: 101, low: 99, close: 100.5, volume: 10 },
      { time: 3000, open: 100.5, high: 102, low: 99.5, close: 101, volume: 10 },
    ];
    const result = resolveOutcome(signal, candles);
    expect(result).not.toBeNull();
