...
describe('detectHammer', () => {
  it('detects hammer with downtrend, RSI <= 40, and confirmation', () => {
    ...
    // Refactor variant A, Фаза 0' группа 3: stop must be anchored to the
    // pattern candle's own low/high (standard price-action convention),
    // not recomputed elsewhere.
    expect(result?.sweepLow).toBe(84);
    expect(result?.sweepHigh).toBe(88.05);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectShootingStar', () => {
  it('detects shooting star with bullish context and confirmation', () => {
    ...
    expect(result?.sweepLow).toBe(110.9);
    expect(result?.sweepHigh).toBe(116);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectPinBar', () => {
  it('detects bullish pin bar near swing low with confirmation', () => {
    ...
    expect(result?.direction).toBe('buy');
    // Refactor variant A, Фаза 0' группа 3: stop beyond the pattern
    // candle's own low/high, not recomputed elsewhere.
    expect(result?.sweepLow).toBe(94);
    expect(result?.sweepHigh).toBe(101);
    expect(result?.oppositeZonePrice).toBeNull();
  });
  ...
  it('detects bearish pin bar near swing high with confirmation', () => {
    ...
    expect(result).not.toBeNull();
    expect(result?.direction).toBe('sell');
    expect(result?.sweepLow).toBe(109);
    expect(result?.sweepHigh).toBe(116);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectHangingMan', () => {
  it('detects hanging man with uptrend, RSI >= 60, and confirmation', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.confirmedByNextCandle).toBe(true);
    expect(result?.sweepLow).toBe(108);
    expect(result?.sweepHigh).toBe(112.05);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectInvertedHammer', () => {
  it('detects inverted hammer with downtrend, RSI <= 40, and confirmation', () => {
    ...
    expect(result?.direction).toBe('buy');
    expect(result?.confirmedByNextCandle).toBe(true);
    expect(result?.sweepLow).toBe(88.45);
    expect(result?.sweepHigh).toBe(92);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectMarubozuBullish', () => {
  it('detects bullish marubozu with volume and trend', () => {
    ...
    // Refactor variant A, Фаза 0' группа 3: stop beyond the candle's own
    // low; target is the classical measured-move projection (close + body
    // height) since no smartMoney zone was supplied to potentially cap it
    // closer.
    expect(result?.sweepLow).toBe(99.9);
    expect(result?.sweepHigh).toBe(106.1);
    expect(result?.oppositeZonePrice).toBeCloseTo(112, 6); // 106 + (106-100)
  });
...
describe('detectMarubozuBearish', () => {
  it('detects bearish marubozu with volume and downtrend', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.sweepLow).toBe(103.9);
    expect(result?.sweepHigh).toBe(110.1);
    expect(result?.oppositeZonePrice).toBeCloseTo(98, 6); // 104 - (110-104)
  });
...
describe('detectBullishEngulfing', () => {
  ...
  it('detects a valid bullish engulfing with full context', () => {
    ...
    expect(result?.volumeConfirmed).toBe(true);
    // Refactor variant A, Фаза 0' группа 3: stop must be anchored to the
    // 2-candle formation's own extreme (min low / max high across both
    // candles), not recomputed elsewhere.
    expect(result?.sweepLow).toBe(96.8);
    expect(result?.sweepHigh).toBe(102.2);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectBearishEngulfing', () => {
  ...
  it('detects a valid bearish engulfing with full context', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.confirmedByNextCandle).toBe(true);
    expect(result?.sweepLow).toBe(97.8);
    expect(result?.sweepHigh).toBe(103.2);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectPiercingLine', () => {
  ...
  it('detects a valid piercing line with >50% penetration and full context', () => {
    ...
    expect(result).not.toBeNull();
    expect(result?.direction).toBe('buy');
    expect(result?.sweepLow).toBe(92.5);
...
describe('detectDarkCloudCover', () => {
  ...
  it('detects a valid dark cloud cover with >50% penetration and full context', () => {
    ...
    expect(result).not.toBeNull();
    expect(result?.direction).toBe('sell');
    expect(result?.sweepLow).toBe(99.5);
    expect(result?.sweepHigh).toBe(107.5);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectTweezerBottom', () => {
  ...
  it('detects a valid tweezer bottom WITH mandatory next-candle confirmation', () => {
    ...
    expect(result?.direction).toBe('buy');
    expect(result?.confirmedByNextCandle).toBe(true);
    expect(result?.sweepLow).toBe(95);
    expect(result?.sweepHigh).toBe(98.5);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectTweezerTop', () => {
  ...
  it('detects a valid tweezer top WITH mandatory next-candle confirmation', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.confirmedByNextCandle).toBe(true);
    expect(result?.sweepLow).toBe(105.5);
    expect(result?.sweepHigh).toBe(110);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectBullishHarami', () => {
  ...
  it('detects a valid bullish harami WITH mandatory next-candle confirmation', () => {
    ...
    expect(result?.direction).toBe('buy');
    expect(result?.confirmedByNextCandle).toBe(true);
    expect(result?.sweepLow).toBe(6.5);
    expect(result?.sweepHigh).toBe(10.5);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectMorningStar', () => {
  ...
  it('detects morning star after downtrend with full favorable context', () => {
    ...
    expect(result?.volumeConfirmed).toBe(true);
    // Refactor variant A, Фаза 0' группа 3: stop must be anchored to the
    // 3-candle formation's own extreme (min low / max high across a/b/c).
    // The fixture's smartMoney OB is same-side (a confirmation zone), not
    // opposite-side, so nearestOppositeZonePrice finds no target — null.
    expect(result?.sweepLow).toBe(87.8);
    expect(result?.sweepHigh).toBe(91.8);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectEveningStar', () => {
  ...
  it('detects evening star after uptrend with full favorable context', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.volumeConfirmed).toBe(true);
    expect(result?.sweepLow).toBe(108.2);
    expect(result?.sweepHigh).toBe(112.2);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectThreeWhiteSoldiers', () => {
  ...
  it('detects three white soldiers with full favorable context', () => {
    ...
    expect(result).not.toBeNull();
    expect(result?.direction).toBe('buy');
    expect(result?.sweepLow).toBe(90.9);
    expect(result?.sweepHigh).toBe(100.1);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectThreeBlackCrows', () => {
  ...
  it('detects three black crows with full favorable context', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.sweepLow).toBe(99.9);
    expect(result?.sweepHigh).toBe(109.1);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectAbandonedBabyBottom', () => {
  ...
  it('detects a valid bullish abandoned baby with strict wick gaps and volume climax', () => {
    ...
    expect(result?.sweepLow).toBe(87.4);
    expect(result?.sweepHigh).toBe(91.6);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectAbandonedBabyTop', () => {
  ...
  it('detects a valid bearish abandoned baby with strict wick gaps and volume climax', () => {
    ...
    expect(result?.direction).toBe('sell');
    expect(result?.confidence).toBeGreaterThanOrEqual(0.65);
    expect(result?.sweepLow).toBe(108.4);
    expect(result?.sweepHigh).toBe(112.6);
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('Rising/Falling Three Methods (continuation.ts, Промт 4)', () => {
  ...
  describe('Rising Three Methods', () => {
    it('detects a valid setup with >=6/9 confluence filters and confidence >= 0.5', () => {
      ...
      // Refactor variant A, Фаза 0' группа 4: stop must be anchored to the
      // low of the 4-candle pre-confirmation formation (impulse + 3
      // consolidation candles), EXCLUDING the 5th (breakout confirmation)
      // candle — computed directly from the same indices the detector uses
      // rather than a hardcoded literal, since this fixture is procedurally
      // generated. Target is the classical measured-move projection.
      const idx = ctx.candles.length - 5;
      const [first, c2, c3, c4, last] = ctx.candles.slice(idx, idx + 5);
      expect(result?.sweepLow).toBe(Math.min(first.low, c2.low, c3.low, c4.low));
      expect(result?.sweepHigh).toBe(Math.max(first.high, c2.high, c3.high, c4.high));
      expect(result?.oppositeZonePrice).toBeCloseTo(last.close + (first.close - first.open), 6);
    });
  ...
  describe('Falling Three Methods', () => {
    it('detects a valid setup with >=6/9 confluence filters and confidence >= 0.5', () => {
      ...
      expect(result!.confluenceFactors!.length).toBeGreaterThanOrEqual(6);
      expect(result!.confidence).not.toBe(0.7);
      // Refactor variant A, Фаза 0' группа 4 — mirror of the Rising case:
      // stop anchored to the HIGH of the 4-candle pre-confirmation
      // formation, target = measured-move projection downward.
      const idx = ctx.candles.length - 5;
      const [first, c2, c3, c4, last] = ctx.candles.slice(idx, idx + 5);
      expect(result?.sweepLow).toBe(Math.min(first.low, c2.low, c3.low, c4.low));
      expect(result?.sweepHigh).toBe(Math.max(first.high, c2.high, c3.high, c4.high));
      expect(result?.oppositeZonePrice).toBeCloseTo(last.close - (first.open - first.close), 6);
    });
