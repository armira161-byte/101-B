...
describe('detectOrderBlockBreaker (OB Breaker Block — role-inversion retest)', () => {
  ...
  it('detects a buy signal on a confirmed bullish-breaker (former bearish OB, now support) wick rejection', () => {
    ...
    const breakTime = candles[candles.length - 6].time;
    const breaker = mockOb({
      top: 105, bottom: 104, time: breakTime, type: 'bullish',
    ...
    // §1: the structural stop is beyond the candle that actually broke the
    // origin OB (breaker.time, found by that exact time in `candles`) — NOT
    // the breaker zone's own top/bottom (105/104), which is a materially
    // different, methodologically wrong level.
    const breakCandle = candles.find((c) => c.time === breakTime)!;
    expect(result?.sweepLow).toBe(breakCandle.low);
    expect(result?.sweepHigh).toBe(breakCandle.high);
    expect(result?.sweepLow).not.toBe(104); // not the breaker zone's own bottom
    expect(result?.sweepHigh).not.toBe(105); // not the breaker zone's own top
  });
...
describe('detectOrderBlockNested (Nested OB — HTF order block containing a fresh M1 order block)', () => {
  ...
  it('detects a buy signal where a fresh M1 bullish order block overlaps a genuine HTF bullish order block and price sits in the confluence zone', () => {
    ...
    expect(result?.direction).toBe('buy');
    // Refactor variant A, Фаза 0' группа 2 (same principle as fvg-nested):
    // stop is anchored to the INNER M1 order block's own boundary — not the
    // outer HTF zone's — and target is the outer HTF zone's own boundary in
    // the trade direction, not an unrelated invented number. Verify against
    // the actually-detected HTF zone (via the exported detectHtfObZones)
    // rather than hardcoding its resampled top/bottom.
    expect(result?.sweepLow).toBe(99.9); // m1Ob.bottom
    expect(result?.sweepHigh).toBe(100.2); // m1Ob.top
    const htfZones = detectHtfObZones(m1);
    const matchedHtf = htfZones.find(
      (z) => z.type === 'bullish' && Math.min(z.top, 100.2) > Math.max(z.bottom, 99.9),
    );
    expect(matchedHtf).toBeDefined();
    expect(result?.oppositeZonePrice).toBe(matchedHtf!.top);
  });
