...
export function detectFvgBreakerBlock(
  ...
  for (const wantType of ['bullish', 'bearish'] as const) {
    ...
    return {
      ...
      // Structural SL/TP inputs — see fvg-return.ts's identical comment and
      // computeFvgTradeLevels in trade-levels.ts. Note: this detector reacts
      // at a REGULAR FVG's own boundary (smartMoney.fvgs filtered by
      // hasBOSConfluence), not an inverted/IFVG zone — same fvg.top/bottom
      // source as fvg-return/fvg-rejection, despite the "breaker" name.
      fvgTop: fvg.top,
