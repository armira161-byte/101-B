...
export function detectBullishEngulfing(ctx: PatternContext): PatternResult | null {
  const { candles, index, structure, htfStructure, session, indicators, smartMoney } = ctx;
  ...
  return {
    ...
    // stop beyond the 2-candle formation's own extreme (standard price-
    // action convention). Target is the nearest opposite structural zone
    // via smartMoney. See computeCandlestickTradeLevels in trade-levels.ts.
    sweepLow: Math.min(prevCandle.low, curCandle.low),
    sweepHigh: Math.max(prevCandle.high, curCandle.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, curCandle.close, direction),
  };
}
...
export function detectBearishEngulfing(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    volumeConfirmed: volumeRatio >= 1.5,
    confirmedByNextCandle: conf.confirmed,
    // Structural SL/TP inputs — see detectBullishEngulfing's identical
    // comment above.
    sweepLow: Math.min(prevCandle.low, curCandle.low),
    sweepHigh: Math.max(prevCandle.high, curCandle.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, curCandle.close, direction),
...
export function detectBullishHarami(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    // Structural SL/TP inputs — same convention as the Engulfing family
    // above (2-candle formation extreme + nearest opposite smartMoney
    // zone). See computeCandlestickTradeLevels in trade-levels.ts.
    sweepLow: Math.min(prevCandle.low, curCandle.low),
    sweepHigh: Math.max(prevCandle.high, curCandle.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, curCandle.close, direction),
  };
...
export function detectBearishHarami(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    time: confirmCandle.time,
    confirmedByNextCandle: true,
    // Structural SL/TP inputs — see detectBullishHarami's identical
    // comment above.
    sweepLow: Math.min(prevCandle.low, curCandle.low),
    sweepHigh: Math.max(prevCandle.high, curCandle.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, curCandle.close, direction),
...
export function detectDarkCloudCover(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    volumeConfirmed: volumeRatio >= 1.5,
    confirmedByNextCandle: conf.confirmed,
    // Structural SL/TP inputs — see detectPiercingLine's identical comment.
    sweepLow: Math.min(prevCandle.low, curCandle.low),
    sweepHigh: Math.max(prevCandle.high, curCandle.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, curCandle.close, direction),
  };
...
export function detectTweezerTop(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    time: confirmCandle.time,
    confirmedByNextCandle: true,
    // Structural SL/TP inputs — see detectTweezerBottom's identical comment.
    sweepLow: Math.min(prevCandle.low, curCandle.low),
    sweepHigh: Math.max(prevCandle.high, curCandle.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, curCandle.close, direction),
  };
