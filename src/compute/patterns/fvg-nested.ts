    ...
    for (const htf of htfCandidates) {
      for (const m1 of m1Candidates) {
        ...
        return {
          ...
          // pattern-audit-checklist.md, "внутрипроектная эвристика, не
          // подтверждаемая внешним источником", sltp-geometry-source-
          // variant-A.md §2): stop is beyond the M1-nested FVG's OWN
          // boundary (m1.bottom/m1.top — the tighter, inner zone), target is
          // the near boundary of the containing HTF zone in the trade
          // direction (htf.top for buy, htf.bottom for sell) — reusing
          // oppositeZonePrice generically as "precomputed structural
          // target", same role it plays for the rest of this pattern
          // family, even though it isn't found via smartMoney here. See
          // computeFvgTradeLevels in trade-levels.ts.
          fvgTop: m1.top,
          fvgBottom: m1.bottom,
