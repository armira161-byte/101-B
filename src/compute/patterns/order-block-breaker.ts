...
import { nextCandleConfirmation, intervalSeconds, nearestOppositeZonePrice } from './pattern-context';
import {
  vwapSideOk,
  rsiConfirmOk,
  ema50AlignedOk,
...
export function detectOrderBlockBreaker(
  ...
  for (const wantType of ['bullish', 'bearish'] as const) {
    ...
    // Structural SL/TP inputs (refactor variant A, Фаза 0' группа 2 —
    // sltp-geometry-source-variant-A.md §1): the stop is NOT the breaker
    // zone's own top/bottom (that's just the origin OB's un-inverted
    // extremes, re-purposed) — it's the candle that actually broke/
    // invalidated the origin Order Block and thereby created this breaker.
    // That candle's time is exactly `breaker.time` (set to `ob.endTime` in
    // smart-money.ts's breaker-generation loop) — the literal liquidity-
    // sweep candle the source doc calls "the tail of the liquidity sweep,
    // not just behind the Breaker body". Left undefined (falling back to
    // the generic estimator downstream) if that candle can't be found in
    // this window — should not normally happen since `candles` is the same
    // series smartMoney was built from, but the origin OB could in
    // principle be older than the candles this call was given.
    const sweepCandle = candles.find((c) => c.time === breaker.time);
