...
describe('detectHarmonicPattern', () => {
  ...
  it('AB=CD: anchors the stop beyond D, not beyond the unrelated earlier zigzag point X (Фаза 0\'\', sltp-geometry-source-variant-A.md п.4)', () => {
    // Same ab-cd fixture as above: X=300(high) A=100(low) B=150(high)
    // C=120(low) D=170(high). ab-cd has no real X-based invalidation
    // anchor — X here is just whatever zigzag point happened to precede A,
    // not a methodological stop level. Before this fix, harmonicStop used
    // min/max(D, X), which for this fixture picked X=300 (far from D=170)
    // instead of D — a stop roughly 130 points away from the actual PRZ
    // instead of a small ATR buffer beyond D.
    const candles = buildSwingCandles(150, [300, 100, 150, 120, 170, 140]);
    const result = detectHarmonicPattern(candles, undefined, undefined, undefined, undefined, TEST_CONFIG);
    ...
    expect(result?.harmonicType).toBe('ab-cd');
    expect(result?.direction).toBe('sell');
    // Sell: stop = D + small ATR buffer — must sit just above D (170), and
    // nowhere near X (300).
    expect(result!.harmonicStop!).toBeGreaterThan(170);
    expect(result!.harmonicStop!).toBeLessThan(200);
  });
