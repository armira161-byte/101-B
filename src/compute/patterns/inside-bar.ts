...
export function detectInsideBar(mother: Candle, inside: Candle, breakout: Candle): PatternResult | null {
  ...
  return {
    ...
    // аудите): та же проблема, что в consolidation-breakout.ts — паттерн
    // молча резолвился через generic ATR×2-фолбэк вместо своей структуры.
    // Стоп — за ПРОТИВОПОЛОЖНОЙ границей материнской свечи (классическая
    // inside-bar-конвенция: полный возврат в диапазон mother отменяет
    // тезис пробоя сжатия, не только касание уровня входа). Цель —
    // measured move: высота диапазона mother, спроецированная от точки
    // пробоя. См. computeInsideBarTradeLevels в decision/trade-levels.ts.
    sweepLow: mother.low,
    sweepHigh: mother.high,
    oppositeZonePrice:
      direction === 'buy' ? breakout.close + motherRange : breakout.close - motherRange,
  };
}
