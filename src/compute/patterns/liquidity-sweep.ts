...
export function detectLiquiditySweep(
  ...
  // Structural SL/TP inputs (refactor variant A, Фаза 0' группа 2 — Group 2
  // wiring): the sweep bar here is `last` itself (unlike
  // liquidity-sweep-reaction, where the sweep happens 1-2 bars before the
  // displacement/confirmation bar) — so the swept extreme IS the current
  // bar's own low/high.
  //
  // sltp-geometry-source-variant-A.md §3: the two setupTypes share the same
  // stop geometry but need DIFFERENT targets — continuation (pure ICT) aims
  // at the next liquidity draw via smartMoney (nearestOppositeZonePrice,
  // same lookup liquidity-sweep-reaction already uses), while
  // reversal-at-key-level (methodologically Wyckoff Spring/Upthrust) aims at
  // the OPPOSITE boundary of the very Trading Range the spring/upthrust
  // happened in — recentHigh/recentLow above, exposed here as
  // rangeHigh/rangeLow. Both are computed unconditionally (cheap, already
  // in scope) so computeLiquiditySweepBaseTradeLevels() in trade-levels.ts
  // can branch on setupType without the detector needing to know which one
