...
import { nearestOppositeZonePrice } from './pattern-context';
...
export function detectMacdDecelerationContinuation(
  ...
  return {
    ...
    // pattern-audit-checklist.md): stop is beyond the deceleration
    // ("pause") candle's own extreme — reusing sweepLow/sweepHigh
    // generically, same fields the rest of this pattern family use — target
    // is the nearest structural level (S/R/OB) in the continuation
    // direction, via smartMoney (same template as
    // computeLiquiditySweepTradeLevels). See
    // computeMacdDecelerationTradeLevels in trade-levels.ts.
    sweepLow: pauseCandle.low,
    sweepHigh: pauseCandle.high,
