...
export function detectMeanReversion(
  ...
  function finalizeConfidence(
    ...
    return {
      ...
      // аудите): та же проблема, что в consolidation-breakout.ts/
      // inside-bar.ts — паттерн молча резолвился через generic ATR×2-
      // фолбэк вместо собственной структуры, хотя у него — единственного
      // из всей таблицы паттернов — есть буквальный, канонический
      // числовой таргет по самому названию стратегии. Стоп — за
      // экстремумом БАРА ВЫХОДА за полосу (prev — тот самый бар, чей
      // depthBeyondBB уже участвует в confidence выше; полный повторный
      // выход за экстремум этого бара отменяет тезис возврата к среднему).
      // Цель — средняя полоса Боллинджера (snapshot.bollingerMiddle) —
      // это и есть "среднее", к которому паттерн по определению
      // стратегии целится, не производная величина, а буквальный смысл
      // названия. См. computeMeanReversionTradeLevels в
      // decision/trade-levels.ts.
      sweepLow: rejectionLow,
      sweepHigh: rejectionHigh,
      oppositeZonePrice: snapshot.bollingerMiddle,
    };
  }
  ...
  if (prev.close < snapshot.bollingerLower && last.close > snapshot.bollingerLower) {
    if (rsiShort !== null && rsiShort < 25) {
      ...
      const result = finalizeConfidence(base, 'buy', prev.low, prev.high);
  ...
  if (prev.close > snapshot.bollingerUpper && last.close < snapshot.bollingerUpper) {
    if (rsiShort !== null && rsiShort > 75) {
      ...
      const returnStrength = clamp01((lastBody / atrValue) / 1.5);
      const base = exitStrength * 0.5 + returnStrength * 0.5;
      const result = finalizeConfidence(base, 'sell', prev.low, prev.high);
      if (result) return result;
    }
  }
