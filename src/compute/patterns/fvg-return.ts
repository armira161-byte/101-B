...
import { intervalSeconds, nearestOppositeZonePrice } from './pattern-context';
import {
...
export function detectFvgReturn(
  ...
  return {
    ...
    // sltp-geometry-source-variant-A.md §2): stop is beyond the FVG zone's
    // OWN far boundary (fvgBottom for buy, fvgTop for sell — deeper
    // penetration than that invalidates the "quick return" thesis this
    // pattern trades), target is the nearest opposite smartMoney zone. See
    // computeFvgTradeLevels in trade-levels.ts (shared by the whole FVG
    // pattern family per the checklist's consolidation instruction).
    fvgTop: fvg.top,
    fvgBottom: fvg.bottom,
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, last.close, direction),
  };
}
