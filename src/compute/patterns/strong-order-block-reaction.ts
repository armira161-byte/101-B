...
import { obFvgConfluenceBonus, intervalSeconds, nearestOppositeZonePrice } from './pattern-context';
...
export function detectStrongOrderBlockReaction(
  ...
  for (const block of activeBlocks) {
    ...
    bestResult = {
      ...
      // block's OWN edge (not recomputed from ATR) — reusing sweepLow/
      // sweepHigh generically the same way order-block-breaker/liquidity-
      // sweep do (whichever of the two the direction actually needs).
      // Target is the nearest opposite smartMoney zone, same template as
      // computeLiquiditySweepTradeLevels. See
      // computeStrongOrderBlockReactionTradeLevels in trade-levels.ts.
      sweepLow: block.bottom,
      sweepHigh: block.top,
      oppositeZonePrice: smartMoney
        ? nearestOppositeZonePrice(smartMoney, last.close, direction)
        : null,
    };
  }
