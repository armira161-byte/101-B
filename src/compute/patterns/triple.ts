...
export function detectMorningStar(ctx: TripleContext): PatternResult | null {
  const { candles, session, indicators, smartMoney } = ctx;
  ...
  return {
    ...
    // stop beyond the 3-candle formation's own extreme (min low / max high
    // across a/b/c), target — nearest opposite structural zone via
    // smartMoney. See computeCandlestickTradeLevels in trade-levels.ts.
    sweepLow: Math.min(a.low, b.low, c.low),
    sweepHigh: Math.max(a.high, b.high, c.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, c.close, 'buy'),
  };
}
...
export function detectEveningStar(ctx: TripleContext): PatternResult | null {
  ...
  return {
    ...
    volumeConfirmed: volumeRatioC >= 1.3,
    confluenceFactors: confluenceList(filters),
    // Structural SL/TP inputs — see detectMorningStar's identical comment.
    sweepLow: Math.min(a.low, b.low, c.low),
    sweepHigh: Math.max(a.high, b.high, c.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, c.close, 'sell'),
  };
}
...
export function detectAbandonedBabyBottom(ctx: TripleContext): PatternResult | null {
  ...
  const { candles, htfStructure, session, indicators, smartMoney } = ctx;
  const n = candles.length;
  if (n < 4 + TREND_LOOKBACK) return null;
  const idx = n - 4;
  const a = candles[idx];
  ...
  return {
    ...
    // Structural SL/TP inputs — see detectMorningStar's identical comment
    // (same convention: 3-candle formation extreme, not including the 4th
    // confirmation candle).
    sweepLow: Math.min(a.low, b.low, c.low),
    sweepHigh: Math.max(a.high, b.high, c.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, c.close, 'buy'),
...
export function detectAbandonedBabyTop(ctx: TripleContext): PatternResult | null {
  ...
  return {
    ...
    volumeConfirmed: volumeFactorValue >= 1.15,
    confirmedByNextCandle: !fourthResult.cancelled && fourthResult.multiplier >= 1.05,
    // Structural SL/TP inputs — see detectAbandonedBabyBottom's identical
    // comment above.
    sweepLow: Math.min(a.low, b.low, c.low),
    sweepHigh: Math.max(a.high, b.high, c.high),
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, c.close, 'sell'),
