...
export function detectPinBar(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    // beyond the pattern candle's own extreme (the dominant wick's tip),
    // target — nearest opposite structural zone via smartMoney. See
    // computeCandlestickTradeLevels in trade-levels.ts.
    sweepLow: patternCandle.low,
    sweepHigh: patternCandle.high,
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, patternCandle.close, direction),
  };
}
