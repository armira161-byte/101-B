...
export function detectHammer(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    // Structural SL/TP inputs (refactor variant A, Фаза 0' группа 3 —
    // pattern-audit-checklist.md, standard price-action convention: stop
    // beyond the formation's own invalidating extreme, not an arbitrary
    // ATR multiple). sweepLow/sweepHigh reused generically (same fields
    // the Group 2 pattern family uses) — for a single-candle reversal the
    // formation IS just patternCandle's own low/high. Target is the
    // nearest opposite structural zone via smartMoney — the checklist's
    // suggestion to "expose obFvgConfluenceBonus's geometry" would only
    // give the SUPPORT zone confirming this entry, not a forward target;
    // nearestOppositeZonePrice is the helper actually built for "next zone
    // ahead in the trade direction" (see liquidity-sweep-reaction.ts).
    sweepLow: patternCandle.low,
    sweepHigh: patternCandle.high,
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, patternCandle.close, 'buy'),
  };
}
...
export function detectShootingStar(ctx: PatternContext): PatternResult | null {
  ...
  return {
    ...
    confluenceFactors: factors.length > 0 ? factors : undefined,
    // Structural SL/TP inputs — see detectHammer's identical comment above
    // (mirrored geometry: stop beyond patternCandle's own high, not low).
    sweepLow: patternCandle.low,
    sweepHigh: patternCandle.high,
    oppositeZonePrice: nearestOppositeZonePrice(smartMoney, patternCandle.close, 'sell'),
  };
}
...
export function detectMarubozuBullish(ctx: PatternContext): PatternResult | null {
  ...
  // Structural SL/TP inputs (refactor variant A, Фаза 0' группа 3): stop
  // beyond the candle's own low (barely any wick by definition of
  // Marubozu). Target is the classical continuation "measured move" —
  // this candle's own body height projected forward from its close — UNLESS
  // a nearer opposite structural zone sits in between, in which case that
  // zone is the more realistic (higher-probability) target per the
  // checklist ("либо структурный уровень, если ближе").
  const measuredMoveTarget = patternCandle.close + body;
  const structuralZone = nearestOppositeZonePrice(smartMoney, patternCandle.close, 'buy');
  const marubozuTarget =
    structuralZone !== null && structuralZone > patternCandle.close && structuralZone < measuredMoveTarget
      ? structuralZone
      : measuredMoveTarget;
  ...
  return {
    ...
    sweepLow: patternCandle.low,
    sweepHigh: patternCandle.high,
    oppositeZonePrice: marubozuTarget,
  };
}
...
export function detectMarubozuBearish(ctx: PatternContext): PatternResult | null {
  ...
  // Structural SL/TP inputs — mirror of detectMarubozuBullish above.
  const measuredMoveTarget = patternCandle.close - body;
  const structuralZone = nearestOppositeZonePrice(smartMoney, patternCandle.close, 'sell');
  const marubozuTarget =
    structuralZone !== null && structuralZone < patternCandle.close && structuralZone > measuredMoveTarget
      ? structuralZone
      : measuredMoveTarget;
