...
export function detectRisingThreeMethods(ctx: ContinuationContext): PatternResult | null {
  ...
  return {
    ...
    // Structural SL/TP inputs (refactor variant A, Фаза 0' группа 4 —
    // pattern-audit-checklist.md): stop beyond the LOW of the whole
    // 5-candle formation EXCLUDING the confirmation candle `last` itself
    // (first + the 3 consolidation candles only — `last` is the breakout
    // confirmation, not part of the invalidating structure). Target is the
    // classical "measure rule" for continuation patterns: impulse candle
    // 1's own body height, projected forward from `last.close` — not a
    // structural zone or ATR×2. See computeThreeMethodsTradeLevels in
    // trade-levels.ts.
    sweepLow: Math.min(first.low, c2.low, c3.low, c4.low),
    sweepHigh: Math.max(first.high, c2.high, c3.high, c4.high),
    oppositeZonePrice: last.close + (first.close - first.open),
  };
}
...
export function detectFallingThreeMethods(ctx: ContinuationContext): PatternResult | null {
  ...
  return {
    ...
    volumeConfirmed: last.volume >= 1.5 * avgVol5,
    confluenceFactors: confluenceList(filters),
    // Structural SL/TP inputs — mirror of detectRisingThreeMethods above.
    sweepLow: Math.min(first.low, c2.low, c3.low, c4.low),
    sweepHigh: Math.max(first.high, c2.high, c3.high, c4.high),
    oppositeZonePrice: last.close - (first.open - first.close),
  };
}
