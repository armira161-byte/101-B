    ...
    for (const htf of htfCandidates) {
      for (const m1 of m1Candidates) {
        ...
        return {
          ...
          // pattern-audit-checklist.md, same principle as fvg-nested.ts):
          // stop is beyond the M1-nested OB's OWN boundary (m1.bottom for
          // buy, m1.top for sell — the tighter, inner zone), target is the
          // near boundary of the containing HTF zone in the trade direction
          // (htf.top for buy, htf.bottom for sell). Reusing sweepLow/
          // sweepHigh generically (this pattern's own "zone edge" is an
          // Order Block, not an FVG, so fvgTop/fvgBottom would be the wrong
          // field name here) and oppositeZonePrice as the generic
          // "precomputed structural target" slot, same roles they play
          // elsewhere in this pattern family. See
          // computeOrderBlockNestedTradeLevels in trade-levels.ts.
          sweepLow: m1.bottom,
          sweepHigh: m1.top,
          oppositeZonePrice: direction === 'buy' ? htf.top : htf.bottom,
        };
      }
    }
  }
