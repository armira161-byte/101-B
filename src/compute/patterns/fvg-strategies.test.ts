...
describe('detectFvgReturn (Strategy A — Возврат к FVG)', () => {
  ...
  it('detects a buy signal on a retest of an unbroken bullish FVG in an uptrend', () => {
    ...
    // §2: stop must be anchored to the FVG zone's own boundary — not
    // recomputed/rounded elsewhere.
    expect(result?.fvgTop).toBe(105);
    expect(result?.fvgBottom).toBe(104);
    // No opposite-side OB/FVG supplied — nothing to target.
...
describe('detectFvgNested (Strategy C — Вложенный FVG)', () => {
  ...
  it('detects a buy signal when an M1 FVG sits inside a same-direction HTF FVG and price is in the overlap', () => {
    ...
    expect(result?.direction).toBe('buy');
    // Refactor variant A, Фаза 0' группа 2: stop is anchored to the INNER
    // M1 FVG's own boundary (102.5/103.2), NOT the outer HTF zone's
    // (102.2/103.7) — target is the HTF zone's near boundary in the trade
    // direction (its top, for a buy).
    expect(result?.fvgBottom).toBe(102.5);
    expect(result?.fvgTop).toBe(103.2);
    expect(result?.oppositeZonePrice).toBeCloseTo(103.7, 6);
  });
});
