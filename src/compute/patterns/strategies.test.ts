...
describe('detectMacdDecelerationContinuation', () => {
  ...
  it('detects a continuation signal on a valid decay-pause-flip shape', () => {
    ...
    // Refactor variant A, Фаза 0' группа 2: stop must be anchored to the
    // deceleration ("pause") candle's own low/high — not an independently
    // invented number. Confirm sweepLow/sweepHigh come from one real
    // candle's actual low/high pair, without hardcoding which index the
    // pause candle sits at internally.
    expect(result?.sweepLow).toBeDefined();
    expect(result?.sweepHigh).toBeDefined();
    expect(candles.some((c) => c.low === result?.sweepLow && c.high === result?.sweepHigh)).toBe(true);
    // No smartMoney supplied here — no opposite-side zone to target.
...
describe('detectStrongOrderBlockReaction', () => {
  ...
  it('detects a bullish reaction with a high score (HTF bias + strong displacement + BOS + Kill Zone)', () => {
    ...
    // Refactor variant A, Фаза 0' группа 2 / sltp-geometry-source-variant-A.md
    // §1: stop must be anchored to the REACTING block's own edge (its
    // bottom, for a bullish/support reaction) — not the breaker/sweep
    // extreme of some other candle, and not recomputed from ATR.
    expect(result?.sweepLow).toBe(100); // block.bottom
    expect(result?.sweepHigh).toBe(107); // block.top
    // No opposite order blocks supplied — no zone to target.
    expect(result?.oppositeZonePrice).toBeNull();
  });
...
describe('detectLiquiditySweep', () => {
  it('detects a bullish sweep: deep wick + volume + swing-level confluence', () => {
    ...
    // Refactor variant A, Фаза 0' группа 2, п.1: the sweep bar IS the
    // current/last bar for this (non -reaction) pattern, so sweepLow/
    // sweepHigh must be exactly this bar's own low/high — used downstream
    // by computeLiquiditySweepTradeLevels() in signal-builder.ts to place
    // a structural stop beyond the swept extreme instead of a flat
    // atrMultiplier*ATR distance from entry.
    expect(result?.sweepLow).toBe(97.6);
    expect(result?.sweepHigh).toBe(100.5);
    // rangeHigh/rangeLow — the same 20-bar lookback window's extremes
    // (WARMUP_HIGH/WARMUP_LOW), needed downstream ONLY for the
    // reversal-at-key-level setupType's target (see next test and
    // computeLiquiditySweepBaseTradeLevels in trade-levels.ts), but exposed
    // unconditionally regardless of which setupType this particular sweep
    // matched.
    expect(result?.rangeHigh).toBe(WARMUP_HIGH);
    expect(result?.rangeLow).toBe(WARMUP_LOW);
    // No order blocks/FVGs supplied here, so there is no opposite-side
    // liquidity zone to target.
    expect(result?.oppositeZonePrice).toBeNull();
  });

  it('reports the nearest opposite-side order block edge as oppositeZonePrice when one exists beyond entry', () => {
    const candles = flatWarmup(30);
    candles.push(candle(30, 99.5, 99.6, 100.5, 97.6, 260));
    const smartMoney: SmartMoneyResult = {
      ...EMPTY_SMART_MONEY,
      // Bearish (resistance) OB above the close — the correct-side target
      // for a bullish sweep's take-profit.
      orderBlocks: [mockOb({ type: 'bearish', top: 110, bottom: 108, time: 20 })],
    };

    const result = detectLiquiditySweep(candles, UP_STRUCTURE, 'london', smartMoney);
    expect(result).not.toBeNull();
    // nearestOppositeZonePrice() for a 'buy' direction uses the OB's near
    // (bottom) edge — see pattern-context.ts.
  ...
  it('allows a reversal-at-key-level sweep against the prevailing trend, right at the structural swing low', () => {
    ...
    // Same Trading Range regardless of setupType — computeLiquiditySweepBaseTradeLevels
    // picks which of rangeHigh/rangeLow to target based on setupType+direction,
    // not the detector.
    expect(result?.rangeHigh).toBe(WARMUP_HIGH);
    expect(result?.rangeLow).toBe(WARMUP_LOW);
  });
