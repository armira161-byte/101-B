...
function evaluateWindow(
  ...
  //
  // Refactor variant A, Фаза 0'' / sltp-geometry-source-variant-A.md, п.4:
  // правило стопа различается по подсемейству, а не одна формула на всё —
  // это НЕ вопрос детекции (геометрия XABCD найдена верно), а вопрос того,
  // какая именно точка инвалидирует тезис:
  //   - Gartley/Bat ("внутренние" — D лежит НЕ дальше X, т.к. AD-ratio <1
  //     XA) — инвалидация именно по X: пробой X отменяет паттерн.
  //   - Butterfly/Crab ("расширяющиеся" — D по определению паттерна лежит
  //     ЗА пределами X, т.к. AD-ratio >1 XA) — X пробит самим паттерном
  //     ещё до входа, это не инвалидация; стоп ставится за самой точкой D.
  //   - ab-cd — своей точки X как якоря инвалидации нет вообще (это
  //     принципиально 4-точечная, не 5-точечная геометрия); используем D.
  // Раньше здесь стояла единая формула `min(D,X)`/`max(D,X)` — для
  // gartley/bat/crab/butterfly она (при корректной геометрии) арифметически
  // совпадает с явным выбором ниже, потому что D у Gartley/Bat всегда лежит
  // между X и A, а у Butterfly/Crab — всегда дальше X по построению. Но для
  // ab-cd точка X — это просто предыдущая zigzag-точка ДО A, не имеющая
  // отношения к этой геометрии, и min/max с ней давал стоп, произвольно
  // "уезжающий" далеко за D всякий раз, когда эта случайная X дальше D —
  // см. регрессионный тест в harmonic-pattern.test.ts.
  const stopAnchorPrice =
    match.harmonicType === 'gartley' || match.harmonicType === 'bat' ? X.price : D.price;
  const currentAtr = latestAtr(candles, snapshot, 14);
  const STOP_ATR_BUFFER = 0.15;
  const harmonicStop =
    direction === 'buy'
      ? stopAnchorPrice - currentAtr * STOP_ATR_BUFFER
      : stopAnchorPrice + currentAtr * STOP_ATR_BUFFER;
