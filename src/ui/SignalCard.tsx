  ...
  return (
    <div className={clsx('rounded-xl border p-3.5 animate-slide-up', sideBg)}>
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-2">
        ...
        {/* Refactor variant A, Фаза 5 (попутно обнаружено и исправлено):
            было signal.expiryBars — этого поля в Signal больше нет с Фазы
            0'/1 варианта А (переименовано). "Тот же параметр, что
            использует резолв исхода" — это Signal.barsToResolve. */}
        <div className="flex items-center gap-1 rounded-lg bg-base-950/50 px-2.5 py-1.5">
          <Timer size={12} className="text-base-400" />
          <span className="text-2xs text-base-400">Экспирация</span>
          <span className="font-mono text-2xs font-semibold text-base-100">
            {formatDurationShort(TIMEFRAME_SECONDS[signal.timeframe] * signal.barsToResolve)}
          </span>
        </div>
      </div>
