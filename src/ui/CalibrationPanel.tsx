...
import { computeFactorStats, computeFactorStatsByTimeframe, MIN_FACTOR_SAMPLES } from '@/ui/factor-analytics';
import {
  computeReliabilitySuggestionsByTimeframe,
  toMultiplierUpdatesByTimeframe,
  breakevenWinRateFromProfitPercent,
  type ReliabilitySuggestionByTimeframe,
} from '@/lib/pattern-reliability-calibration';
import {
  effectiveReliabilityOverridesForSymbolTimeframe,
  applyReliabilityMultiplierUpdatesForSymbolTimeframe,
  resetReliabilityOverridesForSymbolTimeframe,
...
export function CalibrationPanel() {
  ...
  // и передаём производный от него безубыток в computeReliabilitySuggestionsByTimeframe,
  ...
  // РЕФАКТОРИНГ (бинарные опционы, "пересчёт весов/приоритетов паттернов
  // под реально измеренную accuracy по каждому паттерну И таймфрейму"):
  // отдельная разбивка specifично для пересчёта PATTERN_RELIABILITY_
  // MULTIPLIER — factorStats выше (агрегат по всем таймфреймам) остаётся
  // как было для общей таблицы "АНАЛИТИКА ПО ФАКТОРАМ" ниже.
  const factorStatsByTimeframe = useMemo(() => computeFactorStatsByTimeframe(signals), [signals]);
  ...
  const [reliabilityPreview, setReliabilityPreview] = useState<ReliabilitySuggestionByTimeframe[] | null>(null);
  ...
  const previewReliabilityCalibration = () => {
    setReliabilityApplied(false);
    // FIX (аудит калибровки Этапа 2, п.4; расширено на per-timeframe):
    // "before" в предпросмотре берётся из effectiveReliabilityOverridesFor
    // SymbolTimeframe(symbolId, timeframe) — общий дефолт, перекрытый
    // per-symbol, перекрытый per-(symbol, timeframe) override для ТЕКУЩЕЙ
    // пары инструмент+таймфрейм, если она уже была откалибрована ранее.
    setReliabilityPreview(
      computeReliabilitySuggestionsByTimeframe(
        factorStatsByTimeframe,
        (tf) => effectiveReliabilityOverridesForSymbolTimeframe(symbolId, tf),
        breakevenWinRate,
      ),
    );
  };
  ...
  const applyReliabilityCalibration = () => {
    ...
    // Применяем множители ТОЛЬКО к текущей паре (symbolId, timeframe) —
    // другие таймфреймы этого же инструмента и другие инструменты
    // продолжают использовать свой собственный override (или общий
    // дефолт/per-symbol override, если для них калибровка ещё не
    // запускалась). reliabilityPreview может в принципе содержать записи
    // для нескольких таймфреймов (если symbolId торговался на нескольких
    // ТФ) — группируем по timeframe и применяем каждую группу к своему
    // override-слою, а не только к текущему выбранному timeframe, иначе
    // предпросмотр "было/станет" для НЕ-текущего таймфрейма был бы показан
    // пользователю, но не применён при нажатии "Применить".
    for (const [tf, updates] of toMultiplierUpdatesByTimeframe(reliabilityPreview)) {
      applyReliabilityMultiplierUpdatesForSymbolTimeframe(symbolId, tf, updates);
    }
    setReliabilityPreview(null);
  ...
  const handleResetReliability = () => {
    // Сбрасывает override только для текущей пары (symbolId, timeframe) —
    // калибровка других таймфреймов/инструментов не затрагивается.
    resetReliabilityOverridesForSymbolTimeframe(symbolId, timeframe);
    setReliabilityPreview(null);
    setReliabilityApplied(false);
  };
  ...
  const run = async () => {
    ...
    try {
      ...
      // РЕФАКТОРИНГ (бинарные опционы, Фаза 2/4): раньше здесь автоматически
      // применялся калиброванный atrMultiplier к настройкам (setAtrMultiplier)
      // — тот механизм строил SL/TP. Теперь результат (лучший expiryBars и
      // его accuracy) чисто информационный: EXPIRY_BARS — фиксированная
      // системная константа (см. decision/expiry.ts, продуктовое решение
      // "фиксированная экспирация по таймфрейму"), автоприменения нет.
    } catch (err) {
    ...
    <div className="rounded-xl border border-base-800 bg-base-900 p-3">
      ...
      <div className="mt-2.5">
        ...
        <p className="mb-1.5 text-3xs text-base-600">
          ...
          в среднем убыточна. Считается по данным и применяется отдельно к
          каждой паре инструмент+таймфрейм — {symbol?.displayName ?? symbolId}
          на разных таймфреймах может получить разные множители для одного
          и того же паттерна (например, отдельно для {timeframe}, если у
          этого паттерна набралось достаточно сделок именно на нём).
        </p>
                ...
                <table className="w-full text-2xs">
                  <thead className="bg-base-950/60 text-base-500">
                    <tr>
                      ...
                      <th className="px-2 py-1 text-left font-semibold">ТФ</th>
                      <th className="px-2 py-1 text-right font-semibold">N</th>
                  ...
                  <tbody>
                    {reliabilityPreview.map((s) => (
                      <tr key={`${s.name}:${s.timeframe}`} className="border-t border-base-800/60">
                        <td className="px-2 py-1 text-base-300">{s.label}</td>
                        <td className="px-2 py-1 text-base-500">{s.timeframe}</td>
      ...
      {result && !error && (
        <div className="mt-2.5">
          {/* Этап 1 аудита, п.1: раньше "Сделки" здесь и "Сбор данных
              ...
              единая "КАЛИБРОВКА", хотя это отдельная от ML-калибровки
              логрегрессии выше фича. Явная подпись — чтобы это больше не
              читалось как одна и та же готовность.
              РЕФАКТОРИНГ (бинарные опционы, Фаза 2/4): раньше здесь
              подбирался ATR-множитель стопа/цели (форекс-язык, не имеющий
              смысла без SL/TP) — теперь подбирается expiryBars,
              максимизирующий accuracy направления по той же исторической
              выборке, ровно та же модель резолва, что и в проде
              (outcome-scheduler.ts::resolveOutcome). */}
          <div className="mb-1.5 text-3xs font-semibold uppercase text-base-600">
            Бэктест экспирации (отдельно от ML-калибровки выше)
          </div>
          <div className="grid grid-cols-2 gap-2 text-2xs">
            <Stat label="Экспирация (баров)" value={String(result.expiryBars)} />
            <Stat label="Точность" value={`${(result.accuracy * 100).toFixed(0)}%`} />
