...
import { useTickStore, clearOutcomeScheduler } from '@/stores/useTickStore';
    ...
    <div className="flex flex-col gap-3">
      {current ? (
        ...
        <div className="rounded-xl border border-base-800 bg-base-900 p-4 text-center">
          ...
          {/* РЕФАКТОРИНГ ("долго нет сигналов — проверить модуль"): раньше
              здесь всегда была одна и та же фраза-заглушка независимо от
              того, что реально происходит внутри движка — невозможно было
              отличить "рынок сейчас без сетапов" от "гейт X блокирует
              сигнал уже который час подряд" (см. engine.ts::
              lastRejectionReason). Показываем конкретную причину, когда
              она известна, вместо общей фразы про "ожидание конfluence". */}
          <p className="mt-0.5 text-2xs text-base-500">
            {lastRejectionReason ?? 'Ожидание конfluence индикаторов'}
          </p>
        </div>
      )}
