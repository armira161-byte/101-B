import type { Signal, SignalFactorKind, Timeframe } from '@/types/domain';
...
// реально измеренную accuracy по каждому паттерну И таймфрейму"): та же
// самая логика подсчёта, что computeFactorStats выше (та же дедупликация
// STRATEGY_BONUS_PATTERNS, тот же фильтр tradeOpened/outcome), только
// дополнительно разбитая по signal.timeframe, а не только по имени. Раньше
// (computeFactorStats) статистика по паттерну на M1 и, скажем, M15 текла в
// одну и ту же строку — но directional accuracy паттерна не обязана быть
// одинаковой на разных таймфреймах (та же причина, по которой чек-лист
// гипотез горизонта в backtest/pattern-horizon.ts задаёт сетку expiryBars
// отдельно, а не общую для всех паттернов). Отдельная функция (не мутация
// computeFactorStats) — существующая общая таблица "АНАЛИТИКА ПО ФАКТОРАМ"
// (индикаторы/структура/фильтры, не только паттерны) продолжает показывать
// агрегат по всем таймфреймам, как и раньше; эта функция используется
// только для пересчёта PATTERN_RELIABILITY_MULTIPLIER per-timeframe в
// pattern-reliability-calibration.ts.
export interface FactorTimeframeStatRow extends FactorStatRow {
  timeframe: Timeframe;
}

export function computeFactorStatsByTimeframe(signals: Signal[]): FactorTimeframeStatRow[] {
  const resolved = signals.filter((s) => s.tradeOpened !== false && s.outcome !== 'pending');

  const byKey = new Map<
    string,
    { name: string; timeframe: Timeframe; kind: SignalFactorKind; sampleCount: number; decidedCount: number; wins: number }
  >();
  for (const s of resolved) {
    const isDecided = s.outcome === 'win' || s.outcome === 'loss';
    const namesInSignal = new Map<string, SignalFactorKind>();
    ...
    for (const [name, kind] of namesInSignal) {
      const key = `${name}::${s.timeframe}`;
      const existing = byKey.get(key) ?? { name, timeframe: s.timeframe, kind, sampleCount: 0, decidedCount: 0, wins: 0 };
      existing.sampleCount += 1;
      if (isDecided) {
        existing.decidedCount += 1;
        if (s.outcome === 'win') existing.wins += 1;
