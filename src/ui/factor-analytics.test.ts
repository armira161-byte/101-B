...
import { computeFactorStats, computeFactorStatsByTimeframe, MIN_FACTOR_SAMPLES } from './factor-analytics';
...
// набор гарантий, что и у computeFactorStats выше, плюс главная новая
// гарантия — разные таймфреймы одного и того же символа/паттерна не
// смешиваются в одну строку.
describe('computeFactorStatsByTimeframe', () => {
  it('splits the same factor name into separate rows per timeframe', () => {
    const signals = [
      makeSignal({ outcome: 'win', timeframe: '1m', factors: [factor('doji', 'pattern')] }),
      makeSignal({ outcome: 'loss', timeframe: '1m', factors: [factor('doji', 'pattern')] }),
      makeSignal({ outcome: 'win', timeframe: '15m', factors: [factor('doji', 'pattern')] }),
      makeSignal({ outcome: 'win', timeframe: '15m', factors: [factor('doji', 'pattern')] }),
      makeSignal({ outcome: 'win', timeframe: '15m', factors: [factor('doji', 'pattern')] }),
    ];
    const stats = computeFactorStatsByTimeframe(signals);
    expect(stats).toHaveLength(2);

    const oneMin = stats.find((s) => s.timeframe === '1m');
    const fifteenMin = stats.find((s) => s.timeframe === '15m');
    expect(oneMin?.name).toBe('doji');
    expect(oneMin?.decidedCount).toBe(2);
    expect(oneMin?.winRate).toBeCloseTo(0.5, 5);
    expect(fifteenMin?.decidedCount).toBe(3);
    expect(fifteenMin?.winRate).toBe(1);
  });

  it('does not let a high win rate on one timeframe leak into another timeframe of the same pattern', () => {
    const signals = [
      // 1m: consistently loses.
      makeSignal({ outcome: 'loss', timeframe: '1m', factors: [factor('hammer', 'pattern')] }),
      makeSignal({ outcome: 'loss', timeframe: '1m', factors: [factor('hammer', 'pattern')] }),
      // 5m: consistently wins.
      makeSignal({ outcome: 'win', timeframe: '5m', factors: [factor('hammer', 'pattern')] }),
      makeSignal({ outcome: 'win', timeframe: '5m', factors: [factor('hammer', 'pattern')] }),
    ];
    const stats = computeFactorStatsByTimeframe(signals);
    const oneMin = stats.find((s) => s.timeframe === '1m');
    const fiveMin = stats.find((s) => s.timeframe === '5m');
    expect(oneMin?.winRate).toBe(0);
    expect(fiveMin?.winRate).toBe(1);
  });

  it('still deduplicates STRATEGY_BONUS_PATTERNS double-entries within the (name, timeframe) key', () => {
    const signals = [
      makeSignal({
        outcome: 'win',
        timeframe: '5m',
  ...
  it('excludes pending/tradeOpened===false signals, same as computeFactorStats', () => {
    const signals = [
      makeSignal({ outcome: 'pending', timeframe: '1m', factors: [factor('rsi')] }),
      makeSignal({ outcome: 'loss', tradeOpened: false, timeframe: '1m', factors: [factor('rsi')] }),
      makeSignal({ outcome: 'win', timeframe: '1m', factors: [factor('rsi')] }),
    ];
    const stats = computeFactorStatsByTimeframe(signals);
    expect(stats).toHaveLength(1);
    expect(stats[0].sampleCount).toBe(1);
