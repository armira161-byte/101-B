...
export function buildPostMortemReport(signal: Signal): string {
  ...
  lines.push('');
  // Refactor variant A, Фаза 5 (попутно обнаружено и исправлено): этот
  // комментарий и следующая строка описывали СТАРУЮ бинарно-опционную
  // модель ("резолвится по цене закрытия через N баров, без SL/TP") —
  // ровно то, что Фаза 0'/1 варианта А заменила на резолв по реальному
  // касанию stopLoss/takeProfit (см. resolve-by-levels.ts). Заодно были
  // сломаны обращения к полям, которых в Signal больше нет (expiryBars/
  // estimatedPatternMaturityBars переименованы в barsToResolve/
  // recommendedExpiry, см. types/domain.ts).
  lines.push(
    `**Вход:** ${dateStr} · ${directionLabel} · ${formatPrice(signal.entryPrice)} · ` +
    `SL ${formatPrice(signal.stopLoss)} / TP ${formatPrice(signal.takeProfit)} · ` +
    `горизонт резолва ${signal.barsToResolve} бар(ов) (≈${formatDurationShort(TIMEFRAME_SECONDS[signal.timeframe] * signal.barsToResolve)}) · ` +
    `оценка зрелости сетапа ${signal.recommendedExpiry} бар(ов) · ` +
