import type { PatternName, Timeframe } from '@/types/domain';
 ...
 *
 * РЕФАКТОРИНГ (бинарные опционы, п. "пересчёт весов по паттерну И
 * таймфрейму"): реальная directional accuracy паттерна на M1 и, например,
 * M15 — разные величины (иначе не было бы смысла разбивать чек-лист
 * гипотез горизонта по паттернам отдельно от таймфрейма). Добавлен третий,
 * более специфичный слой поверх уже существующего per-symbol —
 * per-(symbol, timeframe). Порядок разрешения (от самого специфичного к
 * самому общему, первое найденное значение побеждает):
 *   per-(symbol, timeframe) override → per-symbol override → глобальный
 *   дефолт (PATTERN_RELIABILITY_MULTIPLIER) → 1.
 * timeframe опционален по той же причине обратной совместимости, что и
 * symbolId — без него просто пропускается самый специфичный слой.
 */
export function getReliabilityMultiplier(name: PatternName, symbolId?: string, timeframe?: Timeframe): number {
  if (symbolId && timeframe) {
    const perSymbolTimeframe = getSymbolTimeframeOverrides(symbolId, timeframe)[name];
    if (perSymbolTimeframe !== undefined) return perSymbolTimeframe;
  }
  if (symbolId) {
...
const SYMBOL_TIMEFRAME_RELIABILITY_OVERRIDE_STORAGE_PREFIX = 'pattern-reliability-multiplier-symbol-timeframe-v1:';
const symbolTimeframeReliabilityOverrides = new Map<string, Partial<Record<PatternName, number>>>();

function symbolTimeframeStorageKey(symbolId: string, timeframe: Timeframe): string {
  return `${SYMBOL_TIMEFRAME_RELIABILITY_OVERRIDE_STORAGE_PREFIX}${symbolId}:${timeframe}`;
}

function symbolTimeframeCacheKey(symbolId: string, timeframe: Timeframe): string {
  return `${symbolId}:${timeframe}`;
}

function loadSymbolTimeframeOverridesFromStorage(symbolId: string, timeframe: Timeframe): Partial<Record<PatternName, number>> {
  const raw = readLocalStorageSafely(symbolTimeframeStorageKey(symbolId, timeframe));
  if (!raw) return {};
  try {
    const parsed: unknown = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return {};
...
function getSymbolTimeframeOverrides(symbolId: string, timeframe: Timeframe): Partial<Record<PatternName, number>> {
  const cacheKey = symbolTimeframeCacheKey(symbolId, timeframe);
  let overrides = symbolTimeframeReliabilityOverrides.get(cacheKey);
  if (overrides === undefined) {
    overrides = loadSymbolTimeframeOverridesFromStorage(symbolId, timeframe);
    symbolTimeframeReliabilityOverrides.set(cacheKey, overrides);
  }
  return overrides;
}

/**
 * Эффективные overrides для пары (инструмент, таймфрейм) — общий дефолт,
 * перекрытый per-symbol, перекрытый per-(symbol, timeframe) там, где они
 * заданы. Используется как "before" в предпросмотре "было/станет" в
 * CalibrationPanel.tsx.
 */
export function effectiveReliabilityOverridesForSymbolTimeframe(
  symbolId: string,
  timeframe: Timeframe,
): Partial<Record<PatternName, number>> {
  return {
    ...PATTERN_RELIABILITY_MULTIPLIER,
    ...getSymbolOverrides(symbolId),
    ...getSymbolTimeframeOverrides(symbolId, timeframe),
  };
}

/**
 * Применяет пересчитанные множители к конкретной паре (инструмент,
 * таймфрейм) — самый специфичный слой, не затрагивает ни глобальный
 * дефолт, ни per-symbol override (используемый как фолбэк для остальных
 * таймфреймов этого же инструмента).
 */
export function applyReliabilityMultiplierUpdatesForSymbolTimeframe(
  symbolId: string,
  timeframe: Timeframe,
  updates: Partial<Record<PatternName, number>>,
): void {
  const cacheKey = symbolTimeframeCacheKey(symbolId, timeframe);
  const current = { ...getSymbolTimeframeOverrides(symbolId, timeframe) };
  for (const [name, value] of Object.entries(updates)) {
    if (typeof value === 'number' && Number.isFinite(value)) {
      current[name as PatternName] = clampReliabilityMultiplier(value);
    }
  }
  symbolTimeframeReliabilityOverrides.set(cacheKey, current);
  writeLocalStorageSafely(symbolTimeframeStorageKey(symbolId, timeframe), JSON.stringify(current));
}

/**
 * Сбрасывает override только для указанной пары (инструмент, таймфрейм) —
 * откатывается к per-symbol override (если есть) или общему дефолту.
 */
export function resetReliabilityOverridesForSymbolTimeframe(symbolId: string, timeframe: Timeframe): void {
  const cacheKey = symbolTimeframeCacheKey(symbolId, timeframe);
  symbolTimeframeReliabilityOverrides.set(cacheKey, {});
  removeLocalStorageSafely(symbolTimeframeStorageKey(symbolId, timeframe));
}
