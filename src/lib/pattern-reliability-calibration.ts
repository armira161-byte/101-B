...
// реально измеренную accuracy по каждому паттерну И таймфрейму"): те же
// формулы (Wilson lower bound → suggestedMultiplierFromWinRate), но на
// вход — computeFactorStatsByTimeframe (factor-analytics.ts) вместо
// computeFactorStats, и выход разбит по timeframe, а не единый список.
// Общий "было/станет" по всем таймфреймам сразу вводил бы в заблуждение:
// паттерн может быть надёжен на M5 и шумен на M1 одновременно — единое
// число смешало бы обе выборки в одну статистику ни о чём конкретном.
// ---------------------------------------------------------------------------

export interface ReliabilitySuggestionByTimeframe extends ReliabilitySuggestion {
  timeframe: Timeframe;
}

/**
 * Тот же алгоритм, что computeReliabilitySuggestions, применённый отдельно
 * к каждому таймфрейму, встретившемуся в factorStatsByTimeframe. Те же
 * критерии исключения строк (MIN_FACTOR_SAMPLES, не PatternName,
 * STRATEGY_BONUS_PATTERNS) применяются per-timeframe — то есть паттерн
 * может пройти порог на M5 (много сделок) и одновременно быть исключён на
 * M1 (мало сделок), это ожидаемо, а не баг.
 */
export function computeReliabilitySuggestionsByTimeframe(
  factorStatsByTimeframe: FactorTimeframeStatRow[],
  currentOverridesByTimeframe: (timeframe: Timeframe) => Partial<Record<PatternName, number>>,
  breakevenWinRate: number = breakevenWinRateFromProfitPercent(DEFAULT_PROFIT_PERCENT_FALLBACK),
): ReliabilitySuggestionByTimeframe[] {
  const suggestions: ReliabilitySuggestionByTimeframe[] = [];
  for (const row of factorStatsByTimeframe) {
    if (row.decidedCount < MIN_FACTOR_SAMPLES) continue;
    if (row.winRate === null) continue;
    if (!isPatternName(row.name)) continue;
    if (STRATEGY_BONUS_NAME_SET.has(row.name)) continue;

    const before = currentOverridesByTimeframe(row.timeframe)[row.name] ?? 1;
    const reliableWinRate = wilsonLowerBound(row.wins, row.decidedCount);
    const after = Math.round(suggestedMultiplierFromWinRate(reliableWinRate, breakevenWinRate) * 100) / 100;
    suggestions.push({
      name: row.name,
...
/** Группирует предпросмотр по timeframe → объект updates для applyReliabilityMultiplierUpdatesForSymbolTimeframe. */
export function toMultiplierUpdatesByTimeframe(
  suggestions: ReliabilitySuggestionByTimeframe[],
): Map<Timeframe, Partial<Record<PatternName, number>>> {
  const byTimeframe = new Map<Timeframe, Partial<Record<PatternName, number>>>();
  for (const s of suggestions) {
    const updates = byTimeframe.get(s.timeframe) ?? {};
    updates[s.name] = s.after;
    byTimeframe.set(s.timeframe, updates);
  }
  return byTimeframe;
}
