...
// РЕФАКТОРИНГ (бинарные опционы, "пересчёт весов/приоритетов паттернов под
// реально измеренную accuracy по каждому паттерну И таймфрейму"): третий,
// самый специфичный слой overrides — те же гарантии изоляции, что и у
// per-symbol слоя выше, но по дополнительному измерению timeframe, плюс
// проверка порядка разрешения all three layers (per-symbol-timeframe >
// per-symbol > global default).
describe('per-(symbol, timeframe) reliability overrides', () => {
  let globalSnapshot: Partial<Record<PatternName, number>>;

  beforeEach(() => {
    globalSnapshot = { ...PATTERN_RELIABILITY_MULTIPLIER };
    resetReliabilityOverridesForSymbol('BTCUSDT');
    resetReliabilityOverridesForSymbolTimeframe('BTCUSDT', '1m');
    resetReliabilityOverridesForSymbolTimeframe('BTCUSDT', '5m');
    resetReliabilityOverridesForSymbolTimeframe('EURUSD', '1m');
  });
  ...
  it('falls back to the global default when neither symbolId nor timeframe is given', () => {
    PATTERN_RELIABILITY_MULTIPLIER.doji = 1.3;
    expect(getReliabilityMultiplier('doji')).toBe(1.3);
  });

  it('a per-(symbol, timeframe) override does not leak into another timeframe of the same symbol', () => {
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '1m', { doji: 1.5 });
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '1m')).toBe(1.5);
    // Same symbol, different timeframe — must be unaffected.
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '5m')).toBe(1);
    // Different symbol, same timeframe — must also be unaffected.
    expect(getReliabilityMultiplier('doji', 'EURUSD', '1m')).toBe(1);
  });

  it('resolves in order: per-(symbol,timeframe) > per-symbol > global default', () => {
    PATTERN_RELIABILITY_MULTIPLIER.doji = 0.7; // layer 3 (global)
    applyReliabilityMultiplierUpdatesForSymbol('BTCUSDT', { doji: 0.9 }); // layer 2 (per-symbol)
    // Without a timeframe-specific override, per-symbol wins over global.
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '5m')).toBe(0.9);
    // Once a timeframe-specific override exists for 1m, it wins for 1m only.
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '1m', { doji: 1.4 }); // layer 1
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '1m')).toBe(1.4);
    // 5m still falls back to the per-symbol layer, untouched by the 1m override.
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '5m')).toBe(0.9);
  });

  it('clamps per-(symbol, timeframe) updates to [RELIABILITY_MULTIPLIER_MIN, RELIABILITY_MULTIPLIER_MAX]', () => {
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '1m', { doji: 99, hammer: -5 });
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '1m')).toBe(RELIABILITY_MULTIPLIER_MAX);
    expect(getReliabilityMultiplier('hammer', 'BTCUSDT', '1m')).toBe(RELIABILITY_MULTIPLIER_MIN);
  });

  it('resetReliabilityOverridesForSymbolTimeframe reverts only that pair, not the per-symbol layer or other timeframes', () => {
    applyReliabilityMultiplierUpdatesForSymbol('BTCUSDT', { doji: 0.9 });
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '1m', { doji: 1.4 });
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '5m', { doji: 0.5 });
    resetReliabilityOverridesForSymbolTimeframe('BTCUSDT', '1m');
    // 1m falls back to the per-symbol layer (0.9), not the global default (1).
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '1m')).toBe(0.9);
    // 5m is untouched by resetting 1m.
    expect(getReliabilityMultiplier('doji', 'BTCUSDT', '5m')).toBe(0.5);
  });

  it('effectiveReliabilityOverridesForSymbolTimeframe merges all three layers, most specific wins', () => {
    PATTERN_RELIABILITY_MULTIPLIER.doji = 0.6;
    PATTERN_RELIABILITY_MULTIPLIER.hammer = 0.7;
    applyReliabilityMultiplierUpdatesForSymbol('BTCUSDT', { doji: 0.9, 'shooting-star': 1.1 });
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '1m', { doji: 1.4 });
    const effective = effectiveReliabilityOverridesForSymbolTimeframe('BTCUSDT', '1m');
    expect(effective.doji).toBe(1.4); // per-(symbol,timeframe) wins
    expect(effective['shooting-star']).toBe(1.1); // falls back to per-symbol
    expect(effective.hammer).toBe(0.7); // falls back to global default
  });

  it('persists per-(symbol, timeframe) overrides in localStorage under a scoped key', () => {
    applyReliabilityMultiplierUpdatesForSymbolTimeframe('BTCUSDT', '1m', { doji: 1.2 });
    const raw = localStorage.getItem('pattern-reliability-multiplier-symbol-timeframe-v1:BTCUSDT:1m');
    expect(raw).toBeTruthy();
    expect(JSON.parse(raw as string).doji).toBe(1.2);
    expect(localStorage.getItem('pattern-reliability-multiplier-symbol-timeframe-v1:BTCUSDT:5m')).toBeNull();
  });
});
