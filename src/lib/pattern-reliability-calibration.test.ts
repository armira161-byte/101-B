...
import type { FactorStatRow, FactorTimeframeStatRow } from '@/ui/factor-analytics';
...
function rowTf(overrides: Partial<FactorTimeframeStatRow>): FactorTimeframeStatRow {
  return {
    name: 'doji',
    timeframe: '1m',
    kind: 'pattern',
...
// реально измеренную accuracy по каждому паттерну И таймфрейму"): тот же
// набор гарантий, что и у computeReliabilitySuggestions выше, плюс главная
// новая гарантия — тот же паттерн на разных таймфреймах получает
// независимые предложения (и независимые "before", если already имеет
// свой override), не смешанные в одну строку.
describe('computeReliabilitySuggestionsByTimeframe', () => {
  it('produces independent suggestions for the same pattern on different timeframes', () => {
    const stats = [
      rowTf({ name: 'doji', timeframe: '1m', decidedCount: 40, wins: 10, winRate: 0.25 }), // weak on 1m
      rowTf({ name: 'doji', timeframe: '15m', decidedCount: 40, wins: 30, winRate: 0.75 }), // strong on 15m
    ];
    const suggestions = computeReliabilitySuggestionsByTimeframe(stats, () => ({}), COIN_FLIP_BREAKEVEN);
    expect(suggestions).toHaveLength(2);
    const oneMin = suggestions.find((s) => s.timeframe === '1m');
    const fifteenMin = suggestions.find((s) => s.timeframe === '15m');
    expect(oneMin!.after).toBeLessThan(1);
    expect(fifteenMin!.after).toBeGreaterThan(1);
  });

  it('excludes rows below MIN_FACTOR_SAMPLES independently per timeframe', () => {
    const stats = [
      rowTf({ name: 'doji', timeframe: '1m', decidedCount: MIN_FACTOR_SAMPLES - 1 }),
      rowTf({ name: 'doji', timeframe: '15m', decidedCount: MIN_FACTOR_SAMPLES }),
    ];
    const suggestions = computeReliabilitySuggestionsByTimeframe(stats, () => ({}), COIN_FLIP_BREAKEVEN);
    expect(suggestions).toHaveLength(1);
    expect(suggestions[0].timeframe).toBe('15m');
  });

  it('excludes non-PatternName factors and STRATEGY_BONUS_PATTERNS, same as computeReliabilitySuggestions', () => {
    const stats = [
      rowTf({ name: 'rsi', timeframe: '1m' }), // not a PatternName
      rowTf({ name: STRATEGY_BONUS_PATTERNS[0], timeframe: '1m' }),
      rowTf({ name: 'doji', timeframe: '1m' }),
    ];
    const suggestions = computeReliabilitySuggestionsByTimeframe(stats, () => ({}), COIN_FLIP_BREAKEVEN);
    expect(suggestions).toHaveLength(1);
    expect(suggestions[0].name).toBe('doji');
  });

  it('reads "before" from a timeframe-specific overrides function, not a single shared value', () => {
    const stats = [
      rowTf({ name: 'doji', timeframe: '1m', decidedCount: 20, wins: 10, winRate: 0.5 }),
      rowTf({ name: 'doji', timeframe: '5m', decidedCount: 20, wins: 10, winRate: 0.5 }),
    ];
    const overridesByTf = (tf: Timeframe) => (tf === '1m' ? { doji: 0.8 } : { doji: 1.2 });
    const suggestions = computeReliabilitySuggestionsByTimeframe(stats, overridesByTf, COIN_FLIP_BREAKEVEN);
    const oneMin = suggestions.find((s) => s.timeframe === '1m');
    const fiveMin = suggestions.find((s) => s.timeframe === '5m');
    expect(oneMin!.before).toBe(0.8);
    expect(fiveMin!.before).toBe(1.2);
  });
});

describe('toMultiplierUpdatesByTimeframe', () => {
  it('groups suggestions by timeframe, each mapping name -> after', () => {
    const stats = [
      rowTf({ name: 'doji', timeframe: '1m', decidedCount: 40, wins: 10, winRate: 0.25 }),
      rowTf({ name: 'hammer', timeframe: '1m', decidedCount: 40, wins: 10, winRate: 0.25 }),
      rowTf({ name: 'doji', timeframe: '5m', decidedCount: 40, wins: 30, winRate: 0.75 }),
    ];
    const suggestions = computeReliabilitySuggestionsByTimeframe(stats, () => ({}), COIN_FLIP_BREAKEVEN);
    const byTf = toMultiplierUpdatesByTimeframe(suggestions);
    expect(byTf.size).toBe(2);
    expect(Object.keys(byTf.get('1m') ?? {})).toHaveLength(2);
    expect(Object.keys(byTf.get('5m') ?? {})).toHaveLength(1);
    expect(byTf.get('1m')?.doji).toBeLessThan(1);
    expect(byTf.get('5m')?.doji).toBeGreaterThan(1);
  });
});
